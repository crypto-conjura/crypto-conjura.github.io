...
\item[(A)] $\kna(q)\le 5\sqrt{\sigma'\delta}+q\delta$ for every $q$, and
  ...
  Unconditional. This settles the statement's assertion that queries whose
...
\item[(B)] $\kappa(q)\le 5\sqrt{\sigma'\delta}+\mu(\min(qM,N^{2}))$ for every $q$
  and every observer. Unconditional.
...
\item[(C$'$)] Hence the conjecture, with the absolute constants $c=2$, $C=8$, for
  ...
  string, or $M\le\sigma'/\sqrt{27\delta}$.
\end{itemize}
...
are reached. This is the one respect in which the route beats a Fourier or
...
terms non-negative so the indicators may be dropped; these are at most
...
Resolution $1$ gives $s\le q$ and $\mu(q)\le q\delta$. For $q=0$, $s=0$ and every
...
\begin{proof}
$5\sqrt{\sigma'\delta}\le5\sqrt{\sigma'q^{+}\delta}$; and $q^{+}\delta\le\sigma'$
...
\begin{proof}
$\varepsilon(D)\le6\sqrt{\sigma'q^{+}\delta}$: in the first branch $D$ has
...
Corollary~\ref{cor:B} and the compression arm. This is not slack in the counting.
