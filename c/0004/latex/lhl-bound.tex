...
\item \textbf{Their random-oracle comparison is query-bounded.} \cite{BDKPPSY11} compares the LHL
against the ``dream'' bound of a cryptographic hash modelled as a random oracle by arguing that
$H(X)$ is distinguishable from uniform only if the attacker queried the oracle at $X$, which for an
attacker of running time $T$ against a source of min-entropy $\mu$ happens with probability at most
$T/2^{\mu}$. In \texttt{main.tex} every party is handed $H$ in full, so there is no query budget to
argue against and that comparison does not reach this setting. The same is true of the
random-oracle extraction literature generally.
\end{itemize}
