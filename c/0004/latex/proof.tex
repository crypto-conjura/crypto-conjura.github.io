% =====================================================================
%  SOLUTION DOCUMENT.  Resolves the public-seed conjecture, which is
%  the whole of the companion statement note main.tex in this folder.
%  The secret-seed conjecture, which is the whole of the companion
%  statement note c/0005/latex/main.tex, is not addressed and remains
%  open.  Along the way, the naive transposition of the secret-seed
%  expression to the public-seed game is shown false
%  (Proposition~\ref{prop:false}), which is what motivates the
%  corrected bound.
%
%  The note opens with an unnumbered "The conjecture, as posed"
%  section reproducing both statements verbatim, one from each of
%  those two files, so this document is readable on its own; this
%  document's own transposed variant then numbers as Conjecture 3.
%  Until 28 August 2026 both conjectures lived in one shared main.tex
%  duplicated into c/0004 and c/0005, and that file's numbering (1 =
%  secret, 2 = public) is what the phrase "Conjecture 1"/"Conjecture
%  2" meant in earlier revisions.  Keep the three files in sync -- if
%  either main.tex changes, that section must be re-copied.
%
...
proven true. Its secret-seed companion, stated in full in
\texttt{c/0005/latex/main.tex}, is not addressed and remains open; a
natural but false transposition of it to the public-seed game is
also disproven here (Proposition~\ref{prop:false}). Both statements
...
The two conjectures below are reproduced verbatim from the two statement notes they come
from, which share a setting and differ only in whether the distinguisher is given the seed:
Conjecture~\ref{conj:stmt-secret} is the whole of \texttt{c/0005/latex/main.tex}, and
Conjecture~\ref{conj:stmt-pub} is the whole of \texttt{main.tex} in this folder, the statement
this note resolves. They are numbered here in that order, secret seed first, because the argument
below reaches the public-seed statement by way of the secret-seed one; the numbering is this
document's own, each of those notes numbering its single conjecture as Conjecture~1. That shared setting is
summarised first and then given in full, in the form used throughout the proof, in
...
This note revisits the secret-seed conjecture of Section~\ref{sec:posed} under the one change that
...
conjecture this note resolves, which already attaches the $1/K$ to the selection term, follows from
