...
%  sections -- The setting, The conjecture, Bibliography -- following
%  exactly the structure and environment conventions of the sibling
%  groth16 statement (Section -> prose facts with \cite -> formal
%  \definition/\conjecture/\remark in the conjecture section ->
%  \aicrbibliography with \bibitem), so the two read as one series.
%
...
%  roadmap ranks closing the factor sqrt(S) gap behind the oblivious
%  base case as "the smallest open quantity in the note and the only
%  one whose resolution needs no new technique in either direction" --
%  strictly easier than BP3' (a larger, harder-to-close factor of q)
%  and BS4-C (which needs a genuinely new combinatorial estimate).
%  BS0+ is kept for that reason; BP3' and BS4-C, and the facts
%  (Joux-Lucks, Guo-Nan-Yao) that existed only to support BS4-C's
%  tightness, are dropped along with the parallel model and the "C
%  disjoint triples" machinery they needed.  The base case itself
%  (formerly a separate boxed Theorem "BS0") is now folded into the
%  facts list as~\ref{F3}, matching groth16's convention of stating
%  prior results as prose facts and reserving the boxed environments
%  for what this document itself defines and conjectures.
%
%  Motivation, proofs, the short-output barrier, and the discarded
%  conjectures are all in main.tex; none of it is repeated here.
%
%  Built on the shared aicr-conjecture.cls house style -- see
