...
Revision r2 of `split-decomp-kappa-2`. It repairs the six findings upheld in a first round of
blind review — every one of them a false or unlicensed claim *about* what the artifact
achieves, or arithmetic inside a justification, and none of them load-bearing — together with
the four items ruled pedantic in the same round. No proof is touched, no result is weakened,
and no new mathematics is added: every change is a deletion, a qualification or a corrected
quotation. Supersedes `split-decomp-kappa-2`; that id's tally does not carry over, having been
recorded against different content.

This artifact attacks **Barrier 1** of `split-decomp-kappa-1-r3` §9 — challenge-steered probes
— and closes it on a region *incomparable* with the region r3 covered for unrestricted
observers: for $q\ge1$ it strictly enlarges, by the factor $1.299\,q^{+}/\sqrt\delta$, the
sufficient condition $M\le\sigma'/\sqrt{27\delta}$ under which r3 covers them, while at $q=0$
r3's hypothesis is met by every $M$ and Theorem E's is not. §7 states the comparison exactly.
It does **not** supersede r3. Its Lemma P and Theorem C are used here and are not reproved; its
Theorems A and B are strengthened, not replaced; its Lemmas 0–2 and Claim 6.1 are reproved from
scratch below so that the new results do not rest on an artifact that has yet to be
blind-verified.

Two further items had been raised by an in-session smell test and repaired in place before that
round: Lemma 0's clause (iii) conjoined the two paired experiments where its proof supplies a
maximiser per pair, and §0's aside on (D) claimed of the collected corollary what holds only of
the display. Neither was load-bearing, and both stand repaired here.
...
$1.302\sqrt{\sigma'\delta}+3.21\delta\sqrt{\sigma'}\le(1.302+3.21)\sqrt{\sigma'\delta}=4.512\sqrt{\sigma'\delta}\le4.9\sqrt{\sigma'\delta}$
...
### END OF ARTIFACT split-decomp-kappa-2-r2 ###
