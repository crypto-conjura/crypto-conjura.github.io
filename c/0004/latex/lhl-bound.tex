...
First a clash of notation. In~\cite{BDKPPSY11}, $u$ is the length of the source, $v$ the number of
extracted bits, $\mu$ the source's min-entropy, $L := \mu - v$ the \emph{entropy loss}, and
$\varepsilon$ the security of the \emph{application} being keyed. That last is not this note's
...
key's length. In the first row that factor is $2^{56.5}$, so at a $77$-bit salt the route certifies
...
witness satisfies both, and on it the query-bounded ceiling is sharper by some twenty orders of
magnitude.
\end{remark}
...
bound, hence $\log_2 j$ further bits of seed in the comparison of Section~\ref{sec:compare}, one or
two in the ranges of interest. So the one-row construction of Theorem~\ref{thm:limit} is
