# split-decomp-kappa-2-r3 — the extraction bounds, after the arm's first blind review

**Revision of `split-decomp-kappa-2-r2` after five independent blind referee passes** — the
**first** this arm has ever had, spanning two model families, lenses: constants and Corollary D″;
Lemma A and Lemma C; Theorem D's scope and Lemma B's union bound; Proposition F and §8's tightness
claims; Theorem E, the gap register and citation fidelity. Two returned CLEAN, three DEFECTS.

**No class-(A) finding was raised by any pass.** No theorem statement changes here, no headline
constant moves, and nothing is reproved. Every repair below is a printed decimal, an unstated
degenerate case, or a sentence that claimed more than the document proves.

## What five independent referees confirmed

- **Corollary D″'s parenthetical is correct, and it is load-bearing.** Its claim that
  $\gamma_0=\delta$ is "already inside the 3.21" was the one point where an earlier pass on the
  *pre-revision* artifact had objected and been **overruled by triage rather than by an
  independent check**. Pass 1 was aimed at that parenthetical without being told any of this and
  confirmed the absorption by tracing $\gamma_0$ through Theorems A⁺/B⁺: it occurs twice in the
  pre-collection display, once additively and once inside $C_0$, and at $\gamma_0=\delta$ the two
  are bounded as $2.498\delta\sqrt{\sigma'}$ and $0.708\delta\sqrt{\sigma'}$, summing to
  $3.2048\le3.21$. Pass 2 confirmed it again, unprompted, as a downstream check. The negation
  matters: without the absorption the leading part is $5.506>5$ and D″'s constant fails outright.
- **Theorem D's scope is not an overclaim.** It is asserted for observers of *unbounded* query
  count, and the danger was specific: the derandomisation lemma's finiteness could have rested on
  "finitely many deterministic $q$-query strategies", which does not survive unbounded $q$ — and
  r3's version of that lemma argues exactly that way. Pass 3 found that this artifact **does not
  cite r3's Lemma 0; it reproves Lemma 0 from scratch with the deduplication step needed**, so
  finiteness rests on non-repetition over a domain of size $N^{2}$ (depth $\le N^{2}$, branching
  $M$, finitely many inputs), not on $q$. Had it leaned on the inherited version the stated scope
  would have been unlicensed. It does not.
- **Lemma A(a)'s Cauchy–Schwarz step needs no independence**, which matters because $m_1$ and
  $m_2$ are both functions of the same $f$ and are in fact perfectly correlated on r3's §1
  counterexample. Two passes checked this: $\mathbb E[XY]\le\sqrt{\mathbb E[X^2]}\sqrt{\mathbb E[Y^2]}$
  holds for any joint law, and $\mathbb E[X^{2}]=\mathbb E[m_1]$ exactly. Verdict CLEAN.
- **Proposition F is CLEAN**, every equality, inequality and boundary case re-derived from first
  principles, including the two traps it was aimed at: challenge-resolution triviality under
  determinism, and the non-independence of the $X_v$ across $v$ (the proof needs only their
  common marginal, invoked through linearity, and correctly never claims more).
- **Lemma B's union bound is exact** — the three exponential factors cancel identically, the
  per-pair residue is $\gamma_0/(2N^{2})$, there are exactly $N^{2}$ pairs, total $\gamma_0/2$ —
  and the quantifier-order move that deletes the revealing rule is valid: the good event carries
  the universal quantifier over $\theta$ *inside* it, so substituting the $f$- and
  $\zeta$-dependent $\theta_{\zeta,f}$ is instantiation, not a smuggled dependence.
- **Theorem E's three hypotheses are each load-bearing**, discharging one summand apiece, and no
  unstated hypothesis is used. Corollary E′'s comparison against r3's Theorem C′ quotes it
  verbatim and correctly reports the two regions as *incomparable* rather than nested.
- **Every citation into the source cards** is stated on the card in the form used; no consequence
  is drawn that the card does not state.

## Repaired here

- **Five printed decimals truncated where an upper bound required rounding up** — raised by pass 1
  and independently by passes 3 and 4, with witnesses at $N=2$, $\sigma=0$ where the intermediate
  bounds are equalities. $\sqrt{1+\ln2}=1.3012099$ against printed $1.3012$;
  $\ln4+3\ln2=3.4657359$ against $3.4657$; $\sqrt{2\ln2}=1.1774100$ against $1.1774$;
  $3\ln2=2.0794415$ against $2.0794$; and the displayed sum $1.3863+2.7726+2.0795=6.2384$ was
  printed as $6.238$. Each is short by $10^{-5}$ to $3\times10^{-4}$, so each inequality is false
  as printed. **Nothing downstream moves**: recomputed, Corollary D′'s leading part is
  $4.1629\sqrt{\sigma'\delta}+1.1774\delta\sqrt M$ against the stated $5$ and $2$, A⁺/B⁺ collect
  to $3.2048\le3.21\le4$, and D″'s $4.5113\le4.9\le5$. Six one-digit edits. (On the last: the
  *inequality* $2C_0\le6.238\sigma'$ is in fact true and loose — the supremum of
  $2C_0/\sigma'$ over admissible $(\sigma,N)$ is $4.852$, at $N=2$, $\sigma=0$ — so only the
  displayed sum was wrong. It is corrected rather than left as a true statement with false
  arithmetic.)
- **$\delta=1$ is now excluded in Theorems A⁺/B⁺** (class (C), passes 1 and 3). Their collected
  forms take $\gamma_0:=\delta$, but $\gamma_0\in(0,1)$ is required by Lemma B and by r3's
  Lemma 3, so at $\delta=1$ the instantiation is inadmissible. The conclusions are true there
  trivially, $4\delta\sqrt{\sigma'}\ge4\sqrt2>1\ge\kappa$, and both Corollary D′ and r3's
  Theorem A dispatch the case first. One clause, and it also removes the same harmless gap from
  D″'s first arm.
- **§9's gap register no longer asserts another artifact's review history** (class (E), passes 3
  and 5). r2 claimed r3 "has no findings file and has not been through blind review". That was
  false when written and is now doubly so. A gap register is not the place for claims about the
  campaign's verification state, and a blind referee cannot check them from a review package.
- **§8's interpolation paragraph is labelled a heuristic** (class (C), pass 5). Its cost formula
  is exhibited without proof and r2 asserted the conclusion with unqualified certainty; nothing
  depends on it. The stray $M(1-o(1))$ is replaced by the constant factor the derivation gives.
- **§8's tightness juxtaposition is corrected** (pass 4). r2 named the open corner as $M$ large
  with $q$ small, then offered Proposition F as evidence that $\delta\sqrt M$ is "individually
  tight" without noting that Proposition F's witness makes $N^{2}$ queries. Since $\kappa$ is
  monotone in $q$, that lower bound constrains $\kappa(1)$ not at all. The paragraph now says so,
  and sharpens the open question accordingly.
- **§1's gloss that Lemma A(a) spends "the second source's independence a second time"** is
  corrected (class (D), passes 3 and 5): the step uses the product structure, via
  $\mathbb E[m_i]\le\delta$, and no independence at all.

## Still owed

Five angles is the lineage standard and this arm now has five — **against r2**. This revision has
had none. Class-(D) items from passes 3 and 5 that are pure cross-referencing or wording, and do
not affect a claim, are left for a later editorial pass rather than churned here.

Two gaps no pass could close, both recorded rather than repaired. r3's Theorem C and Lemma P are
taken as GIVEN and this arm does not reprove them. And **no source card has ever been checked
against the paper it summarises**, so [CDGS, Claim 2] and [CDGS, Claim 3] — which enter through
r3's Lemma P — rest on transcription.
...
$\mu'(s)=\min(s\delta,2(s\delta^{2})^{1/3},1)$. §6 records how the two compare.
...
$q^{+}\delta\le\sigma'$ and $M\le\sigma'q^{+}/(4\delta)$. For $q\ge1$ this enlarges by the factor
$1.299\,q^{+}/\sqrt\delta$ the sufficient condition $M\le\sigma'/\sqrt{27\delta}$ under which r3
covers unrestricted observers; §7 records where the two regions are instead incomparable.

**(F)** On the family $\delta=1/N$, $2\le M\le N^{2}/2$, the new term $\delta\sqrt M$ is tight
to within a factor $12$, already for resolution-$1$ observers: on that family no $q$-free bound
can improve either the exponent $\tfrac12$ of $M$ or the coefficient $2$ by more than $8\sqrt2$.
Off that diagonal nothing is claimed.
...
$\sqrt\delta$ better. This is a second use of the *product structure*, not of independence,
...
$(\mathsf{Real},\mathsf{Real}_0)$ or $(\mathsf{Real},\mathsf{Dec})$, the latter taken with
respect to a family $\mathcal Y=\{Y_{f,\zeta}\}$ fixed before $D$ is quantified. For every
...
deterministic strategy may be assumed never to repeat a query: the oracle is the fixed function
$f$ for the whole of a run, so $D'$ may answer any repeated position from its own transcript of
earlier answers, which changes no output and only lowers the query count. It is then a finite
decision tree of depth at most $\min(q,N^{2})$, branching over $[M]$, whose behaviour matters
only on inputs in $[M]\times\mathcal Z$, the experiments never feeding it a $\zeta$ outside the
support of $\mathbf z$. All of $[M],\mathcal Z,[N]^{2}$ being finite, there are finitely many such
...
unpredictability parameter. In the flat family of §8, $m_1=m_2=1/N=\delta$ identically and
...
**Corollary B$^{*}$.** (The name is local; r3's Corollary B$'$, quoted in §7, is a different
...
$M\le k_1k_2$, while $t_{C_1}\ge\sqrt{(M\ln2)/(2k_1k_2)}$. §8 turns this into a proof. What is
...
$\sqrt{2\delta L}\le1.3013\sqrt{\sigma'\delta}$; and, using $\ln(1/\delta)\le\ln N$,
$$2C_1=2M\ln2+2\ln4+4\ln N+2\ln\tfrac1\delta\le1.3863M+2.7726+6\ln N\le1.3863M+3.4658\sigma',$$
whence $\delta\sqrt{2C_1}\le1.1775\delta\sqrt M+1.8617\delta\sqrt{\sigma'}\le1.1774\delta\sqrt M+1.8617\sqrt{\sigma'\delta}$,
...
$(1+1.3013+1.8617)\sqrt{\sigma'\delta}+1.1775\delta\sqrt M\le5\sqrt{\sigma'\delta}+2\delta\sqrt M$. $\square$
...
from r3, which uses $\sqrt{2\delta(L+C_0)}$ and $\mu$. For the collected forms assume $\delta<1$ — at $\delta=1$ the
conclusions hold trivially, $4\delta\sqrt{\sigma'}\ge4\sqrt2>1\ge\kappa$, and $\gamma_0:=\delta$ would
violate the $\gamma_0\in(0,1)$ that Lemma B and r3's Lemma 3 both require — and take
$\gamma_0=\delta$, so $\ln(1/\delta)\le\ln N$ and
$2C_0\le1.3863\sigma+5.5452+6\ln N\le(1.3863+2.7726+2.0795)\sigma'\le6.2384\sigma'$, giving
...
Against r3's $\kappa^{\mathrm{na}}(q)\le5\sqrt{\sigma'\delta}+q\delta$ the second term is never
larger, since $\mu'(q)\le q\delta$; and the first is smaller for $\delta\le0.85$, the
$\sqrt{\sigma'}$ now carrying $\delta$ rather than $\sqrt\delta$. The first comparison does not
hold for every $\delta<1$: the replacement carries the extra additive $\sqrt{2\delta\ln(eN)}$,
which reverses it near $\delta=1$.
...
$1.3013\sqrt{\sigma'\delta}+3.21\delta\sqrt{\sigma'}\le(1.3013+3.21)\sqrt{\sigma'\delta}=4.5113\sqrt{\sigma'\delta}\le4.9\sqrt{\sigma'\delta}$
for the first, using $\delta\sqrt{\sigma'}\le\sqrt{\sigma'\delta}$; the term $\gamma_0=\delta$ is
not a further summand, being already inside the $3.21$ that Theorems A⁺, B⁺ supply.)
...
**Corollary E′ (what is gained over r3's Theorem C′).** r3's Theorem C$'$ has, verbatim, the
hypothesis: *“Suppose $N\ge2$, $q^{+}\delta\le\sigma'$, $P\le\sqrt{\sigma'/\delta}$, and either
$D$ has challenge resolution $1$, or the hypothesis of Corollary B$'$ holds — in particular if
$M\le\sigma'/\sqrt{27\delta}$.”* It then reaches the same conclusion as Theorem E, with the same
$c,C$ and the same conditions on $P$ and $q$. Theorem E requires neither branch.

*What is strictly enlarged.* The enlargement claimed here is over the *in particular* clause
$M\le\sigma'/\sqrt{27\delta}$, which is a sufficient condition for r3's second branch and not
equivalent to it. That clause is strictly contained in Theorem E's region, since
$\sqrt{27\delta}\ge4\delta$ for $\delta\le1$ gives
$\sigma'q^{+}/(4\delta)\ge\sigma'/(4\delta)\ge\sigma'/\sqrt{27\delta}$, and the ratio of
thresholds is $\frac{\sqrt{27}}{4}\cdot\frac{q^{+}}{\sqrt\delta}=1.299\,q^{+}/\sqrt\delta$.

*What is not.* r3's second branch is the hypothesis of its Corollary B$'$, verbatim
*“$q^{+}\delta\le\sigma'$ and either $M\le\sqrt{\sigma'/(q\delta)}$ or
$M\le\sigma'^{3/2}\sqrt{q/\delta}/27$”*, which the *in particular* clause only implies. Against
that hypothesis containment fails: at $q=0$ its first arm holds for every $M$, so r3 covers
every $M$ there, while Theorem E requires $M\le\sigma'/(4\delta)$. The two regions are
therefore incomparable, not nested, and nothing here claims containment of r3's hypothesis as
r3 states it. (r3's Corollary B$'$ is unrelated to §4's Corollary B$^{*}$, which is a statement
about statistical distance.)

*The first branch is dropped, not subsumed:* it covers all $M$ for resolution-$1$ observers.
What is now known is the union: under the standing hypotheses $P\le\sqrt{\sigma'/\delta}$ and
$q^{+}\delta\le\sigma'$, the conjecture holds with $c=2$, $C=8$ for (i) every observer when
...
*Corollary D′ is tight in $M$ to within a factor $12$ on the family $\delta=1/N$.* On this
family Corollary D′ gives $\kappa(N^{2})\le5\sqrt{\sigma'\delta}+2\delta\sqrt M$ and
Proposition F gives $\kappa(N^{2})\ge\delta\sqrt M/(4\sqrt2)$; the $M$-terms differ by
$8\sqrt2<12$. So **on this family no $q$-free bound can improve the $M$-dependence of Theorem D
by more than $8\sqrt2$** — both the exponent $\tfrac12$ of $M$ and the coefficient $2$ are
pinned there. The family fixes $\delta=1/N$ and $2\le M\le N^{2}/2$, and licenses nothing off
that diagonal.
...
$M=\lfloor N^{2}/2\rfloor$ the advantage is at least $\frac18\sqrt{1-1/N^{2}}$ — exactly
$\frac18$ for even $N$ — while
...
Proposition F — and nothing here claims their minimum is.

**But note where that tightness sits, because r2's juxtaposition invited a misreading.** The
corner just identified as open is $M$ large with $q$ *small*, whereas Proposition F's witness
$D^{*}$ makes $N^{2}$ queries — the opposite extreme. Since $\kappa$ is monotone in $q$, a
$q$-query observer being free to ignore queries, a lower bound at $q=N^{2}$ constrains
$\kappa(1)$ **not at all**; and at $q=N^{2}$ the condition $M\le\sigma'q^{+}/(4\delta)$ is
essentially free. So $\delta\sqrt M$ is demonstrated tight only at large $q$, and **nothing in
this document, or in r3, gives any evidence that $\kappa(q)$ carries $M$-dependence in the corner
where the $M$-cap actually binds.** That sharpens the open question above: before asking whether
the first arm can be made $M$-free in general, ask whether $\kappa(q)$ has *any* $M$-dependence
at small $q$ — exhibit a $q$-query lower bound carrying $M$ for $q\ll N^{2}$, or prove the
$M$-free bound there. A referee raised this independently against r2. An interpolation between the two
routes, revealing the runs for $v$ in a subset $T\subseteq[M]$ and union-bounding over the
$2^{M-|T|}$ completions, costs $\mu'(q|T|)+\delta\sqrt{2(C_0+(M-|T|)\ln2)}$ and appears to give
nothing beyond the endpoints up to constants: driving the first term below $1$ forces
$|T|\lesssim1/(q\delta^{2})$, which leaves $M-|T|\ge\tfrac78M$ at $q=1$ whenever
$\delta\sqrt M\gtrsim1$.

**This paragraph is a heuristic, not a theorem, and r2 stated it with unqualified certainty.** The
cost formula is exhibited without proof; no Lemma or Proof is offered for it, and none of
Theorems D, E, Corollaries D′, D″ or Proposition F depends on it. A blind referee checked the
arithmetic that follows from taking the formula on faith and found it internally consistent up to
constants, which is evidence that the sketch is not wrong — not that it is established. r2 also
wrote $M(1-o(1))$ where the derivation gives a constant factor, and nothing in this setting tends
to a limit.

## 9. Gap register

- **[INHERITED: r3 Theorem C, r3 Lemma P]** — *load-bearing*, for **Theorem E and Corollary E′
  only**. Theorems D, A⁺, B⁺, Corollaries D′, D″ and Proposition F do not depend on it. Taken as
  GIVEN and not reproved; a reader of this document alone should treat these two as hypotheses.
  (r2 asserted that `split-decomp-kappa-1-r3` "has no findings file and has not been through
  blind review". That was already false when written — r3 has a findings file, verdict CLEAN —
  and is now doubly so: its six class-(C) findings have been triaged and repaired in
  `split-decomp-kappa-1-r4`. A blind referee separately flagged the sentence as unverifiable from
  a review package, since no package file evidences another artifact's review history. Claims
  about the campaign's verification state do not belong in a gap register and are not made here.)
...
### END OF ARTIFACT split-decomp-kappa-2-r3 ###
