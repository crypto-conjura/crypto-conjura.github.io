...
What is recorded is what the collision route gives on that game, and what it cannot.
On a fixed universal table the leftover hash lemma returns the conjectured bound's first summand
exactly (Proposition~\ref{prop:univ}). No table available at the seed lengths at issue is universal
(Proposition~\ref{prop:nouniv}), and on the tables that are available the lemma returns
...
it costs $\log_2 W$ bits of seed against the conjectured $\log_2\log_2 D$. Bounding the parties'
oracle queries then separates two routes that the unbounded game conflates: the collision route
survives, governed by the number of \emph{rows} the source touches rather than by its query count
(Theorem~\ref{thm:qlhl}), while the distinguisher's budget cannot enter it at all and is priced
instead by an identical-until-bad argument (Theorem~\ref{thm:badcell}). Theorem~\ref{thm:three}
states all three budgets together, and Proposition~\ref{prop:qp} shows the predictor's must be at
least the distinguisher's. Section~\ref{sec:prior} records that the floor itself is \emph{not new}
--- it is the seed-length limitation of the leftover hash lemma, known since Stinson and stated as
such by Barak et al.~\cite{BDKPPSY11} --- and says what the note adds instead.
Section~\ref{sec:not} says what is \emph{not} claimed.
\par\smallskip\noindent\emph{Shape.} Section~\ref{sec:setting} fixes the game and the notation,
Section~\ref{sec:bounds} states every bound with no proof, Section~\ref{sec:compare} compares them,
and Section~\ref{sec:proofs} proves them. A reader who wants only the results needs the first
three.}
...
Unqualified, $\epsilon$ means $\epsilon_\infty$: the table-holding predictor. That is the
hypothesis under which every bound of this note is proved save Theorem~\ref{thm:badcell}, and
Theorem~\ref{thm:three} is where it is shown to be forced rather than merely convenient.
...
This section states every bound the note establishes, and the one it does not. Nothing is proved
here; the proofs are collected in Section~\ref{sec:proofs}, in the order the statements appear. The
one bound with no proof anywhere in this note is the first, which is \texttt{main.tex}'s conjecture
and is proved in \texttt{proof.tex} beside it.
...
needs $K \ge D/R$; the uniform $H$ of Section~\ref{sec:setting} is not one.
...
In the order the statements appear in Section~\ref{sec:bounds}. Theorem~\ref{thm:qlhl}'s proof needs
one lemma of its own, which is stated where it is used.
...
\section{Notes on the bounds}\label{sec:notes}

Five readings that belong to particular statements above rather than to the argument as a whole.
