...
The whole difference is the **selection** term, and the obstruction there is structural rather than statistical. The family $\{H(k,\cdot)\}$ is not universal and cannot be made so, so $\gamma$ must be taken over the supports the source may pick *after* seeing $H$ -- and those can be chosen to make it large. Fix seeds $k_1,\dots,k_j$ and symbols $r_1,\dots,r_j$ and let the source be uniform on
$$T := \{\, x : H(k_i,x) = r_i,\ i = 1,\dots,j \,\}, \qquad |T| \approx D/R^{j},$$
which is admissible, since the source reads the whole table and the seed is drawn afterwards. On $T$ those $j$ rows are **constant**, contributing their maximum to the collision probability: $\gamma \ge (R-1)j/K$. Confining $j$ rows to a symbol set of size $R/s$ instead gives $\gamma \ge (s-1)j/K$ with $|T| \approx D/s^j$, and $(s-1)/\log_2 s$ increases, so $s = \min\{R, D\epsilon\}$ is best. With $L := \min\{\log_2 R,\ \log_2 D - \log_2(1/\epsilon)\}$ the lemma cannot yield better than
$$\tfrac12\sqrt{\;\epsilon R \;+\; \frac{\bigl(\log_2 D - \log_2(1/\epsilon)\bigr)\bigl(2^{L}-1\bigr)}{L\,K}\;}.$$

**The rate in $K$ is the same square root the conjecture asks for.** What the lemma loses is a *factor* of about $\sqrt{2^L/L}$, which is $L - \log_2 L$ extra bits of seed: for $D = 2^{256}$, $R = 2^{64}$ and a source of min-entropy $128$, reaching $\delta = 2^{-32}$ needs $\log_2 K \approx 69.5$ under the conjectured bound and $127$ by this route. The [Radhakrishnan--Ta-Shma optimum](https://doi.org/10.1137/S0895480197329508) is the former.

The loss is the Cauchy--Schwarz step $\|v\|_1 \le \sqrt{KR}\,\|v\|_2$ inside the lemma. Support selection concentrates the deviation $v$ on $j$ of the $K$ rows -- those rows become point masses -- rather than spreading it over all $KR$ cells, and on such a $v$ that step is off by $\sqrt{RK/j}$: the true advantage for the source above is $\approx j/K$, while the lemma returns $\approx \tfrac12\sqrt{Rj/K}$. That is why the optimal-extractor proofs work in $L_1$ instead, by concentration plus a union bound over flat sources. The [c/0004](/c/0004/) proof takes that route, arriving at it independently. (What is stated here is a limitation: the lemma cannot do better than the displayed bound. Whether it does that well is not claimed.)
