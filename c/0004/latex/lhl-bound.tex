...
Its question is a comparison: measured against the bounds already known for extracting from a
random oracle, what does the conjectured bound buy? The answer is \emph{seed length}, and the
margin is large. The leftover hash lemma's collision route cannot certify a target $\delta$ at all
without about $\log_2 R$ bits of seed on top of the $2\log_2(1/\delta)$ both routes pay
(Theorem~\ref{thm:limit}); a $2$-universal family does not exist below $D/R$ seeds
(Proposition~\ref{prop:nouniv}); the unsalted random-oracle heuristic of \cite{BDKPPSY11} needs no
seed at all, but is refuted outright by a source that reads the oracle
(Proposition~\ref{prop:unsalted}); and pricing the adversary's queries instead costs
$\log_2 q$ bits (Theorem~\ref{thm:badcell}). Against these the conjecture asks for
$\log_2\log_2 D + 2\log_2(1/\delta) + 2$ --- $77$ bits where the collision route needs $190$,
universal hashing $1920$ and the query-bounded route $113$, in the leading example --- and it is the
only one of them whose seed grows with neither the length of the derived key, nor the length of the
secret beyond a doubly-logarithmic term, nor the adversary's query budget.
\par\smallskip\noindent\emph{Shape.} Section~\ref{sec:setting} fixes the game,
Section~\ref{sec:bounds} states the bounds without proof, Section~\ref{sec:compare} is the
comparison and is the point of the note, and Section~\ref{sec:not} says what is not claimed. The
appendices carry the proofs and everything peripheral to the comparison, including
Appendix~\ref{sec:prior}, which records that the \emph{floor} of Theorem~\ref{thm:limit} is not new
--- it is the leftover hash lemma's known seed-length limitation, attributed by \cite{BDKPPSY11} to
Stinson. What is being claimed as new here is the comparison, not that floor.}
...
\medskip
\noindent\textbf{Query budgets.} Sections~\ref{sec:bounds} onwards also treat the variant in which
each party is given oracle access in place of the table: $S^{H}$ makes at most $q_S$ queries before
returning $(x,z)$, the predictor at most $q_P$, and $\mathsf{D}^{H}(\sd,y_b,z)$ at most $q_D$. The
seed is still drawn after $S$ terminates. Writing $Q_S \subseteq \calK\times\calD$ for the set of
cells the source queries, two features of a source govern what follows, and neither is $q_S$:
...
Unqualified, $\epsilon$ means the strongest of these: the predictor is unbounded and holds the whole
table. Every bound below is proved under it save Theorem~\ref{thm:badcell}, which needs only a
predictor as rich as the distinguisher; Appendix~\ref{sec:predictor} shows that hypothesis is forced
...
Every bound the comparison of Section~\ref{sec:compare} draws on, stated and not proved; the proofs
are in Appendix~\ref{sec:proofs}. Two are quoted from elsewhere: the conjecture, proved in
\texttt{proof.tex} beside this note, and the results of \cite{BDKPPSY11}. Material that does not
bear on the seed-length comparison --- the collision route's behaviour under query bounds, the
predictor's own budget, and the longer discussions --- is in the appendices.
...
Section~\ref{sec:compare} and Section~\ref{sec:prior} measure against several bounds
of~\cite{BDKPPSY11}, so those are listed here with the rest. All are quoted rather than proved, and
all live in that paper's model, not the game of Section~\ref{sec:setting}: there $\mathrm{Ext}$ is a
fixed public function, the source is a distribution rather than an algorithm, and the random oracle
appears in no definition, lemma or theorem. Remark~\ref{rem:roquery} sets the two models side by
side.

One clash of notation to clear first. In~\cite{BDKPPSY11}, $u$ is the length of the source, $v$ the
number of extracted bits, $\mu$ the source's min-entropy, $L := \mu - v$ the \emph{entropy loss},
and $\varepsilon$ the security of the \emph{application} being keyed. That last is not this note's
...
pseudorandom generator and extracting with the result does \emph{not} work in general --- they give
a DDH-based generator and a perfectly universal hash whose output is distinguishable with
probability close to $1$ given the short seed --- but it is sound when the number of extracted bits
is logarithmic in the generator's security, and sound in minicrypt, so any counterexample needs a
...
Items 2, 3 and 5 are about \emph{entropy loss} and items 1 and 4 about \emph{seed length}, and the
two axes are independent. It is worth being explicit about which axis this note is on: the seed
length. Their entropy-loss improvement, item 3, leaves item 1 exactly where it was; their own
attempt on item 1 is item 4, and it fails in general. That is the sense in which the seed-length
limitation is the more stubborn of the two, and it is the one Theorem~\ref{thm:limit} is about.
...
Against a universal table the second summand vanishes.
...
If $\{h(k,\cdot)\}_{k\in\calK}$ is $2$-universal then $D \le RK$, that is $K \ge D/R$. So no
...
distinguisher's budget at all --- it bounds a statistical distance, which is by definition the
supremum over every distinguisher --- so pricing $q_D$ means leaving the lemma. What one gets by
leaving it is the following, and it is the bound the seed comparison of
Section~\ref{sec:compare} uses at a bounded budget. (What becomes of the collision route itself
under query bounds is Appendix~\ref{sec:rowbreadth}, and it does not change the seed comparison.)
...
following needs no probability and no queries at all.
...
inputs, hence $\epsilon$-unpredictable with $\epsilon \le R/D$, with $r_S = s_S = 1$ --- and a
distinguisher making \emph{no} oracle queries whatever, such that
\[
...
More generally, for $1 \le j \le K$ the source of Proposition~\ref{prop:qtight} --- which pins $j$
rows at a uniform input and leaks their values --- has $s_S = j$ and is $\epsilon$-unpredictable
...
$s_S = 1$; but each pinned row costs $\log_2 R$ bits of min-entropy, so a source required to retain
min-entropy $\mu$ can afford only $j \le (\log_2 D - \mu)/\log_2 R$ of them.
\end{proposition}
...
Remark~\ref{rem:qd} gives; by Corollary~\ref{cor:which} it is the smallest of them whenever
...
that zero is the most informative entry in the row, and because it is bought rather than free. Two
hypotheses buy it. The first is a distinguisher of running time $T \le \delta\,2^{\mu}$ --- that is
$T \le 2^{224}$, $2^{448}$ and $2^{236}$ in the three rows. Those ceilings are enormous, which is
why the heuristic is attractive in practice; but they are ceilings, and the whole of
\texttt{main.tex} lives above them, where every party reads the entire table and $T/2^{\mu}$ says
nothing.

The second hypothesis is that the source is independent of $H$, and \emph{that} zero does not
survive dropping it --- not approximately, and not by a little. By
Proposition~\ref{prop:unsalted}, at $K = 1$ a source that reads $H$ and is uniform on the largest
fibre of the single row, of min-entropy $\log_2 D - \log_2 R$, is distinguished from uniform with
advantage $1 - 2^{-m}$ by a distinguisher making \emph{no query at all}. That source is admissible
in every row of the table --- its min-entropy is $1920$, $1792$ and $384$ against the $256$, $512$
and $300$ the rows ask for --- and its entropy loss is $L = \mu - v = 1792$, $1536$ and $256$, so
the heuristic predicts $\epsilon_{\mathrm{RO}} \le 2^{-1792}$, $2^{-1536}$ and $2^{-256}$ where the
truth is $1 - 2^{-128}$, $1 - 2^{-256}$ and $1 - 2^{-128}$. The estimate is not merely inapplicable
off its hypothesis; it is wrong by as much as an estimate of a probability can be.
...
(Proposition~\ref{prop:nouniv}), and it is included because it makes the point of Section~\ref{sec:vs}
...
at $n = 44$, $76$ and $74$ in the three rows. So at $2^{80}$ --- the budget one would actually
quote --- bounding the queries buys nothing here: it asks for $113$ bits where the conjecture asks
for $77$, and for $145$ against $141$ and $139$. The query bound helps only for adversaries weaker
than $2^{44}$ or so, and the reason is visible in the two exponents: the bad-cell route pays
$\log_2 q$ where the conjecture pays $\log_2\log_2 D$, and only halves the $\delta$ term in exchange.
...
advantage $s_S/K$ is attained by pinning $s_S$ rows at the source's own output --- but each pinned
row costs $\log_2 R$ bits of min-entropy, so the three rows admit only
...
Where the slack comes from is worth naming, since it says what a sharper theorem would have to do. A
source may query $(k,x)$ at its own output for a great many $k$ and simply \emph{ignore} the
...
alphabet of size $R$ with minimum distance at least $(1 - 1/R)K$. The Plotkin bound, in the form
``$d = (1-1/q)n$ implies $|C| \le qn$'', gives $D \le RK$. For the proviso, if $R \ge D$ take
...
For the general case, let $S$ draw $x \sample \calD$, query $(k,x)$ for $k < j$ and output
$\bigl(x, z = (H(0,x),\dots,H(j-1,x))\bigr)$; Proposition~\ref{prop:qtight} gives
$\epsilon \le R^{j}/D$ and $s_S = j$, the latter because the source queries its own output in
exactly those $j$ rows. Let $\mathsf{D}$ make no query and return $1$ exactly when $\sd < j$ and
$y_b = z_{\sd}$. Under $b = 0$, $y_0 = H(\sd,x) = z_{\sd}$ whenever $\sd < j$, so the probability
is $j/K$; under $b = 1$ it is $j/(KR)$. The entropy remark is $\log_2(R^{j}/D) \le -\mu$ rearranged.
\end{proof}
...
\emph{weakens} the hypothesis --- a predictor that cannot read the table is easier to defeat --- so
...
unbounded by construction --- Remark~\ref{rem:qd} --- so the predictor must be unbounded too, and
\emph{that} is why every bound of this note other than Theorem~\ref{thm:badcell} is stated against
a predictor holding the whole table. It is not a stylistic choice of the strongest hypothesis; it is forced by the
technique.
...
bound, hence $\log_2 j$ further bits of seed in the comparison of Section~\ref{sec:compare} ---
one or two in the parameter ranges of interest --- so the one-row construction of Theorem~\ref{thm:limit} is essentially the
whole effect.
...
exist.} At the parameters of Section~\ref{sec:compare}'s first row --- a Diffie--Hellman element in a
...
Proposition~\ref{prop:nouniv}'s $K \ge D/R$, which is Stinson's linear-in-$u$ result reached by another
route. So the two salt columns of Section~\ref{sec:compare}'s table are the two branches of one bound
...
all, at a min-entropy of $\log_2 D - \log_2 R$. So this comparison needs the source independent of $H$ \emph{itself} --- a strictly
