...
TOPIC_DESCRIPTIONS = {
    ...
    "random-sources": "How many parties in a private protocol must be able to toss coins at all, as a resource separate from how many coins are tossed.",
    "incompressible-encryption": "Encryption whose ciphertexts cannot be meaningfully compressed, and the bounded-storage adversaries it defends against.",
    "pseudorandom-quantum-states": "Efficiently generated families of quantum states indistinguishable from Haar-random ones, and the properties of the Haar ensemble their analysis needs.",
    "backdoored-primitives": "Primitives whose public parameters hide a backdoor for their designer, and the constructions that immunize against one.",
    "pseudorandom-codes": "Error-correcting codes whose codewords are computationally indistinguishable from random strings, and the assumptions that give them.",
    "watermarking": "Marking the output of a generative model so it can be detected later, undetectably and robustly to editing.",
    ...
    "minimum-circuit-size-problem": "The Minimum Circuit Size Problem, its multi-output and gap variants, and their role in meta-complexity.",
    "orthogonal-vectors": "The k-Orthogonal Vectors problem and its planted variants, central to fine-grained complexity and its cryptographic applications.",
    "interactive-oracle-proofs": "Interactive oracle proofs, including succinct IOPs whose communication is close to the witness length.",
    "batch-verification": "Proving many statements at once more cheaply than proving each separately, while preserving the proof system's guarantees.",
    "space-bounded-computation": "Space as a counted resource, and space-time trade-offs for circuit evaluation and satisfiability.",
