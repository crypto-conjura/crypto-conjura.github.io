# split-decomp-kappa-3-r2 — reducing Theorem E's hypotheses, and locating the $P$-cap

**Revision of `split-decomp-kappa-3` after blind review** (`split-decomp-kappa-3-findings.md`,
verdict DEFECTS, one finding F1 of class A, upheld). F1: Corollary G1 was titled "the proved
region now contains the Contract's own instantiation" while its proof used
$P=\lfloor t\rfloor$, whereas `thm:main` instantiates at $P=\lceil t\rceil$, which
**violated** hypothesis (H1) as then stated — at 165 of 180 grid points. The repair, below,
widens (H1) by one, which is free: the query term and the additive $\delta$ are each at most
$\tfrac1{16}\sqrt{\sigma'q^{+}\delta}$ in the only regime the conclusion is non-vacuous, so
the total falls from exactly $8$ to $7\tfrac18$ and $C=8$ survives with room. Corollary G1
now uses `thm:main`'s literal $\lceil t\rceil$, and its own constant improves from $17$ to
$13$ because $\lceil t\rceil\ge t$ where $\lfloor t\rfloor$ was only $\ge t/2$.

Exactly three places changed: the (G1) bullet of §0, hypothesis (H1) and the last two terms of
Theorem E″'s proof in §2, and Corollary G1. §§1, 3, 4 and 5 are **byte-identical** to
`split-decomp-kappa-3`; §6 differs only in the id on its END line and in swapping the standard
inequality $\lfloor t\rfloor\ge t/2$, now unused, for $\lceil t\rceil\le t+1$. Theorem E″'s
statement is unchanged apart from (H1), and Lemma G0, Corollary G2 and the whole of §4 are
untouched — the review accepted all of them, including the quantifier-order Remark, which was
the flagged class-(A) hazard.
...
### END OF ARTIFACT split-decomp-kappa-3-r2 ###
