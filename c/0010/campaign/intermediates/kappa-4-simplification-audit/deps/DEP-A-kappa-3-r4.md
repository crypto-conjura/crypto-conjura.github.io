# split-decomp-kappa-3-r4 — reducing Theorem E's hypotheses, and locating the $P$-cap

**Revision of `split-decomp-kappa-3-r3` after a single audit finding.** No mathematics changes
anywhere: no theorem, lemma, corollary, hypothesis or constant is touched, and no proof step is
altered. One Remark stated the wrong window and is corrected.

- **A3 (class A).** Remark `rem:window` of §2, new in r3, wrote the set uncovered by `conj:main`
  as $\sqrt{\sigma'q^{+}/\delta}+1<P<N^{2}$ and then said "the window is where every application
  lives". **The two claims are inconsistent.** That lower endpoint is the *per-$q$* hypothesis
  **(H1)**; `thm:main` instantiates at $P=\lceil t_q\rceil$ with
  $t_q:=\sqrt{\sigma'q^{+}/\delta}$, and $\lceil t_q\rceil\le t_q+1$ — which is precisely what
  Corollary G1, three paragraphs earlier in the same section, is written to verify. So under the
  endpoint as written the application point is outside the window by construction, and the
  "every application" clause is false of it.

  The clause is true of `conj:main`'s own window, whose lower endpoint is $q$-free because
  **(H1)** intersected over $q$ binds at $q=0$:
  $$\sqrt{\sigma'/\delta}+1\ <\ P\ <\ N^{2}.$$
  Every application with $q\ge1$ lies in *that* window, strictly: non-vacuity forces
  $t_0:=\sqrt{\sigma'/\delta}>8\sigma'\sqrt{q^{+}}\ge16$, and $t_q=t_0\sqrt{q+1}$, so
  $\lceil t_q\rceil\ge t_0\sqrt2>t_0+1$ for $q\ge1$, while $\lceil t_q\rceil<N^{2}$. At $q=0$ the
  endpoints coincide and the point is covered — `conj:main` at $q=0$ restated. **Corrected** in
  §2, with the reasoning stated rather than the endpoint merely swapped.

  This is exactly the per-$q$ / intersection-over-$q$ conflation that r3's own finding A2 was
  written to prevent, committed three paragraphs after A2 states it, in a Remark added by the
  same revision. Recording it that way is the point: the distinction is slippery enough that the
  document establishing it did not survive its own next page.

  Two consequences worth stating, since the correction moves the boundary the wrong way for the
  campaign. The gap is **wider** than r3 claimed, not narrower — the uncovered window grows by
  $[\,t_0+1,\ t_q+1\,]$ — so r3 understated the remaining work. And nothing is weakened by the
  repair: Theorem E″, **(H1)**, **(H2)**, Lemma G0, Corollary G1 and Corollary G2 are all
  untouched and all still hold exactly as stated in r3.

**Invariant, checked mechanically section by section rather than asserted.** Against
`split-decomp-kappa-3-r3`: **§0, §1, §3, §4 and §5 are byte-identical.** **§6** differs in
exactly one line, the id on its END line. **§2** differs in exactly one paragraph, Remark
`rem:window`, which is replaced; every other byte of §2 — Theorem E″ and its proof, both
Remarks on the slack and on what changed, the quantifier-order Remark, Corollary G1 with its
title and scope note, the A2 Remark and the deflationary note on (G1) — is unchanged.

The numeric claims in the replaced Remark — that $t_0>8\sigma'\sqrt{q^{+}}$ and $t_0\ge16$ in
every non-vacuous instance, that $\lceil t_q\rceil>t_0+1$ for every $q\ge1$, that
$\lceil t_q\rceil<N^{2}$, and that the window is non-empty — were machine-checked over **1261
non-vacuous points** spanning $N\le2^{30}$, $\sigma\le64$, $q\le10^{7}$, $\delta\in[1/N,1]$:
**zero failures**. The check is not prose: it is `c/0010/campaign/checks/rem-window-r4.py`, which
prints its own grid size and worst margins and is the first executable evidence committed in this
campaign. Its grid is this document's, not the 189-point grid r3 reports for its own claims; no
attempt was made to reproduce that one. Two margins are worth recording. The worst ratio
$t_0/(8\sigma'\sqrt{q^{+}})$ is $1.0059$ — the bound is essentially tight, as it must be, being
read straight off the non-vacuity inequality — while the worst margin
$\lceil t_q\rceil-(t_0+1)$ is $129.5$, so the applications are not near the boundary of the
window but well inside it.

Everything r3 recorded about its own provenance still applies and is not repeated here; for the
four findings F1–F4 of the r2 blind review, and audit items A1 and A2, see
`split-decomp-kappa-3-r3.md`. The verification status is unchanged and remains **zero passes of
five**: r3 was never refereed, so r4 inherits an unreviewed lineage at this arm.
...
every $D$ — and for every $M$, so this upper end is unconditional where the lower end is not. So
`conj:main` is trivially true at $P\ge N^{2}$, and on the $P$-axis the set it leaves uncovered is
the window
$$\sqrt{\sigma'/\delta}+1\ <\ P\ <\ N^{2},$$
non-empty in every non-vacuous instance, since $8\sqrt{\sigma'q^{+}\delta}<1$ and $\delta\ge1/N$
force $\sqrt{\sigma'/\delta}\le\sqrt{\sigma'q^{+}/\delta}<N/8$.

**The endpoint is the $q$-free one, and writing the per-$q$ one here is the trap A2 was raised
about — finding A3.** The lower endpoint belongs to `conj:main`, so it is **(H1)** *intersected
over $q$*, which binds at $q=0$; it is not the per-$q$ $\sqrt{\sigma'q^{+}/\delta}+1$ of **(H1)**
itself. The per-$q$ endpoint would contradict Corollary G1 three paragraphs above: `thm:main`
instantiates at $P=\lceil t_q\rceil$ with $t_q:=\sqrt{\sigma'q^{+}/\delta}$, and
$\lceil t_q\rceil\le t_q+1$ is exactly what G1 verifies, so the application point lies *inside*
**(H1)** and cannot lie in a window bounded below by it.

Against the correct, $q$-free endpoint the applications do fall outside, and it is worth seeing
why sharply: **every application with $q\ge1$ lives in this window.** Non-vacuity gives
$\delta<1/(64\sigma'q^{+})$, hence $\sigma'/\delta>64\sigma'^{2}q^{+}$ and
$t_0:=\sqrt{\sigma'/\delta}>8\sigma'\sqrt{q^{+}}\ge16$, using $\sigma'\ge2$. Since
$t_q=t_0\sqrt{q+1}$, for $q\ge1$
$$\lceil t_q\rceil\ \ge\ t_q\ \ge\ t_0\sqrt2\ >\ t_0+1,$$
because $t_0(\sqrt2-1)>16\cdot0.414>1$; and $\lceil t_q\rceil\le N/8+1<N^{2}$, so the application
point is interior. At $q=0$ the two endpoints coincide and $\lceil t_0\rceil\le t_0+1$ is covered,
which is `conj:main` at $q=0$ restated. So the reason the applications sit outside the proved
region is *not* that $\lceil t\rceil$ is too large for **(H1)** — Corollary G1 settled that — but
that `conj:main` demands one family good at every $q$ and is therefore pinned at $q=0$, where the
$\sqrt{q^{+}}$ relaxation is worth nothing.

This bounds the problem but does not shrink it. The window is the $P$-axis alone; the $M$-corner
$M>\sigma'/(2\delta)$ is a separate uncovered region, and neither contains the other.
...
### END OF ARTIFACT split-decomp-kappa-3-r4 ###
