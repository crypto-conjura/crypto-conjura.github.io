...
\[
  ...
  \;=\; \frac{W-1}{K},
  \qquad\text{so the lemma returns at least}\quad
  \frac12\sqrt{\frac{W-1}{K}} .
\]
This holds for \emph{every} table, not merely a typical one. Pinning $j$ rows rather than one
...
$KR$ cells, and on such a $v$ that step is off by about $\sqrt{RK}$: for the source above the true
advantage is $O(1/K)$ while the lemma returns $\tfrac12\sqrt{(W-1)/K}$. So the conjecture is not a
