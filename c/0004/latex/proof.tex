...
%  The secret-seed conjecture is not addressed and remains open.
%  Along the way, the naive transposition of the secret-seed
...
%  section reproducing both statements verbatim, so this document is
%  readable on its own; this document's own transposed variant then
%  numbers as Conjecture 3.  That section is now the site's only
%  record of the secret-seed statement, and is load-bearing for that
%  reason: Proposition~\ref{prop:false} is about an expression
%  derived from it, so the statement has to be printed here for the
%  proposition to mean anything.
%
%  History of the two statements, because the numbering moved twice.
%  Until 28 August 2026 both conjectures lived in one shared main.tex
%  duplicated into c/0004 and c/0005, numbered 1 = secret and 2 =
%  public there; that is what "Conjecture 1"/"Conjecture 2" means in
%  earlier revisions of this file and on the pages.  That note was
%  then split so each folder stated one conjecture, and c/0005 was
%  removed from the site later the same day, leaving main.tex in this
%  folder as the only statement note.  Keep the two files in sync --
%  if main.tex changes, that section must be re-copied.
%
%  Built on the shared aicr-solution.cls house style (inherits
%  aicr-conjecture.cls's fonts/colors, widened to a book-trim page)
...
proven true. Its secret-seed companion, which no longer has a
...
reproduced verbatim; it is the statement this note resolves.
