...
- [ ] **Run the harvester over `latex/harvest/` and promote what survives (~size unknown — this is the largest item in the file; batch it, don't run it all at once)**
  ...
  - **What's actually waiting**, counted 19 August 2026: 403 PDFs across 16 authors' folders (Yuval Ishai 77, Geoffroy Couteau 52, Aggelos Kiayias 50, Stefano Tessaro 47, Prabhanjan Ananth 39, Benny Applebaum 27, Qipeng Liu 23, Alex Lombardi 20, Alexander Russell 18, Divesh Aggarwal 13, Luowen Qian 12, Jens Groth 11, Siyao Guo 8, Noah Stephens-Davidowitz 6; Boaz Barak and Dominique Unruh folders exist but are empty). 49 papers already went through the pipeline (`--report` has the log) and left 48 draft folders in `latex/conjectures/`; `scripts/draft_status.py --report` says 46 of those are already promoted to `c/` and exactly **two are not**: `polynomial-compatibility` and `simon-oracle-simultaneous-inversion` — the smallest possible next step, before touching any of the 403.
  - **Promotion is the part that isn't automated**, and it's real per-draft judgment, not a script: follow `latex/README.md` § "Publishing a draft" — create `open-problems/<topic>/<slug>/` (or the paper equivalent), transcribe the statement into `index.qmd`, and hand-write `draft: <folder-name>` under `problem:` in the new page, since folder names and `problem:` slugs never match automatically and `groth16`/`split-nilp` are byte-identical drafts belonging to different pages. Two undocumented pre-commit-hook requirements bit the 18 August batch twice: an empty status-marker block for `scripts/status_badge.py`, and (check `latex/README.md` for the second one before assuming the commit will just go through).
  - **This is exactly the tension the "Credibility and distribution" section opens with.** 403 unread PDFs could plausibly produce well over 100 new conjectures, every one landing with `proof_review: ai` on a site where that's already true of all 29 existing statements and none has been read by a person. Don't run `--all` in one sitting: harvest and promote in small batches, and weigh this against the steering-member-review and prior-resolution-sweep tasks above rather than letting it silently outrun them.
