...
- [ ] **UC Encyclopedia content — one entry left, and it needs a library card rather than a cryptographer (~5min once someone has Springer or IEICE access; the other 103 are done as of 28 August 2026)**

  **Closed 28 August 2026.** `scripts/uc_status.py --check` reports **89 Defined, 1 Not yet written, 14 No canonical definition** across the 104 pages, against 79 / 19 / 6 at the start of that run and 55 / 46 / 3 when this entry was written on 18 August. Run the script for the current split rather than trusting any number written here.

  **The one entry still open is `f-ibe`, and it is not a research gap.** Nishimaki, Manabe and Okamoto define $\mathcal{F}_{\mathsf{IBE}}$ and prove UC-secure IBE equivalent to IND-ID-CCA2-secure IBE (VIETCRYPT 2006, LNCS 4341, pp. 337–353; journal version IEICE Trans. Fundamentals E91-A(1):262–271, 2008). Neither version is obtainable — not on ePrint, searched by title, author and result; Springer, IEICE and J-STAGE all paywalled; the authors' own publication list offers no full text. The page records what is known, marks the citation `[UNVERIFIED]`, and says so. **Anyone with institutional access to Springer or IEICE can close this in five minutes**, and that is the whole of the remaining work on the encyclopedia's content.

  **What the 28 August run settled, beyond the entries themselves.** Three of these were open questions this file had recorded rather than answered.

  - **The 60-line box ceiling is lifted.** This file asked for "a decision about how a box longer than 60 lines is presented", and the answer is `\begin{interface}[nofloat, breakable]` with no `multicols`. Past about sixty numbered lines two columns overflow the book's 5.5×8.5in page and lose the tail *silently* — the failure `f-cred` hit at 87 lines. `breakable` runs the box onto a second page instead; it does not work inside `multicols`, which is the trade. Verified against a synthetic 91-line fragment: all 91 numbers print this way, 44 in the old form. It rides on the `interface` environment's existing optional argument, so it costs no edit to `ucgamers.sty` and no rebuild. The six entries passed over on size — `f-secmsg` (73 lines), `f-cke` (65), `f-cgka` (106), `g-ledger` (65), `f-chan` (75), `f-vote` (56) — are all written.
  - **The `f-ledger` / `g-ledger` duplicate is resolved**, and both pages now say so to each other.
  - **The "stub titles are usually wrong" finding held to the end.** `f-rp` said "Ideal cipher, two-sided permutation" and the printed object is a deliberate *relaxation* of the ideal cipher; `f-cke` said "Continuous KE", which is Alwen–Coretti–Dodis's primitive and not the epoch key exchange its own citation prints; `f-pir` said "Private information retrieval" where the box is *verifiable* PIR; `f-fhe` said "Threshold FHE decryption", which belongs to `f-thdec`. Four more retitles, and three status corrections — `f-prf-f-prp`, `f-gc` and `f-fhe` all read "emerging" for objects that have no formulation in progress at all.

  **Three new source defects, all transcribed or repaired with the page saying which.** `f-secmsg` records a counter (`rcv_msgnum`) that its functionality initializes and reads and never increments — transcribed as printed, since repairing it means choosing where the increment goes. `f-cgka`'s `Propose` asserts on a variable that operation never binds, and the paper's own main-body version of the same operation has no such line — transcribed as printed for the same reason. `f-abe`'s `InitAttr` is guarded with the wrong polarity, making the operation unreachable and the functionality unable to issue a first key — **repaired**, because the source's own protocol fixes the direction, exactly as `f-asyncmpc`'s never-firing counter was.

  **A sixth false-negative mode for a box scan: prose with no environment at all.** `f-oram`'s functionality is two bullets under a section heading — no figure, no numbered environment, no box. Add that to `f-dkg`'s numbered `Functionality` environment and `g-ledger`'s named block. `f-pir`'s is printed as a numbered *Algorithm*, which a figure-caption sweep also misses.

  **The shading trap has a second instance, and it is the benign one — which is the distinction to check for.** `f-vote`'s source captions its functionality with a colour key. Rendered and inspected: unlike OPAQUE, where shading *selected* between three functionalities and a text-only reading produced a blend, here the colours *annotate* one functionality, tagging each clause with the security property it enforces. Every clause is present in every reading; only the labels are lost. So the rule is not "shading means danger" but "find out which kind of shading it is" — and in the benign case the labels are worth recovering, since the source has already done the work of saying which line carries which property.

  **Two tooling bugs, both fixed or documented.** `scripts/uc_status.py`'s field regex used `\s*$` under `re.M`, which ate the trailing newline and fused `definition:` into the frontmatter's closing fence — it fires only on a page the script rewrites, so 104 entries went by without it surfacing, and it would have corrupted every stub as it was written. Fixed. And `gen_interface.py`'s `\Req` parser takes a braced argument and discards the rest of the source line, so `\Req{...}; \ $call$` compiles in LaTeX and silently drops the call from the HTML — a drift `--check` cannot see, because both sides come from the same parse. Not fixed; documented in `g-ledger.tex`'s header and here. **Read the generated line map against the fragment**, not just the gate output. Same for `\opsig` arguments, which are stripped of math: keep them to plain names.

  **Three LaTeX facts worth not rediscovering.** `\opl` and `\fopl` put their argument into a hyperref target, so `\text{-}` inside them fails to compile — and is unnecessary, since `\op` typesets in text mode where a plain hyphen is already a hyphen. `\bits` already expands to `\{0,1\}^*`, so `\bits^{p(\lambda)}` is a double superscript. And a `\State` must sit on one source line.

  **The declared-fragment treatment generalized.** `f-crypto`, `f-cred` and `f-dh` established it for oversized boxes; `f-cgka` extends it to a functionality with sixteen starred helpers across two figure-pages and two recursive safety predicates, all carried as declared parameters and named on the page — which is how the source presents them too. `f-chan` declares its two recursive time functions the same way.

  **A keyword this encyclopedia did not have, and now does.** `f-cgka`'s source distinguishes `req`, which unwinds state and returns ⊥, from `assert`, which "permanently halts, making the real and ideal worlds trivially distinguishable" and exists "to validate inputs of the simulator". The second is a proof obligation written into the code, not a runtime check, and rendering it as `require` would have turned the paper's four negative theorems into statements about nothing. Kept as a distinct keyword on that page.

  **What the historical record below is for.** Everything from here to the end of this entry is the 18–23 August runs' findings, kept because they are the reason the process works and because several of them are still live warnings for anyone editing these pages. The numbers in them are superseded by the count above.
...
- [ ] **Give `c/0004` and `c/0005` the prior-art section they have never had (~1.5h)**

  Found 28 August 2026 while answering "why doesn't this already follow from known results on the LHL for the RO?". It largely does, and neither page says so. `c/0004/latex/proof.tex` cites exactly one work in its bibliography (McDiarmid); `c/0004/index.qmd`'s `sources:` list is `main.tex` plus McDiarmid. There is no Scout record, no literature check, and no related-work paragraph on either page. For a page carrying the site's only non-open badge that is the wrong gap to have.

  **The correspondence, checked 28 August 2026.** Put `n = log_2 D`, `k = log_2(1/eps)`, `m = log_2 R`, `d = log_2 K`. Balancing each summand of the conjectures against a target `delta` gives: Conjecture 2 (public seed) `<-> m = k - 2log(1/delta)`, `d = log n + 2log(1/delta)`; Conjecture 1 (secret seed, `c/0005`) `<-> m = k + d - 2log(1/delta)`. These are, respectively, Theorem 6.17 and Theorem 6.14 of Vadhan, *Pseudorandomness* (Foundations and Trends in TCS; published December 2012 version at people.seas.harvard.edu/~salil/pseudorandomness/), the classical probabilistic-method existence theorems for optimal strong and ordinary extractors. The match is not merely of shape. Theorem 6.17 states `d = log(n-k) + 2log(1/eps) + O(1)`, and `eq:main-sharp` of `proof.tex` carries `ln(2eD*eps) ~ (n-k)ln2`, so it delivers `log(n-k)` exactly; the note's `b_t := (1/t)ln C(D,t)` is the textbook count of flat k-sources, `C(2^n, 2^k)`. So `eq:main-sharp` *is* Theorem 6.17 with explicit constants, and `eq:main-plain` is the same theorem with the crude estimate `C(D,t) <= (eD)^t`.

  **The LHL itself is not the relevant precedent, and the page should say why.** The family `{H(k,.)}` must be 2-universal for the LHL to apply. Vadhan's note to Theorem 6.18 is explicit that universal hashing needs seed length `Omega(n)`, i.e. `K >= D^Omega(1)` (pairwise independence needs `K >= D`), and calls it "far from optimal". The conjectures' regime is `K ~ log_2 D`, where no universal family exists at all. The natural repair — condition on `H = h`, bound the fixed family's collision probability, apply an almost-universal LHL — fails structurally: collision probability is an `L_2` quantity and converting it to statistical distance costs a factor `R` inside the square root, so a deviation of true size `sqrt(log D / K)` comes out as `(R log D / K)^(1/4)`. That is why the optimal-extractor proofs work in `L_1` (Chernoff/McDiarmid plus a union bound over flat sources) — which is what Lemmas 3.2--3.3 of `proof.tex` already do, arriving independently at the right technique.

  **What is genuinely not in the literature, and is what the pages should claim.** Three things. (1) The unpredictability hypothesis is an *average* over `H`: `E_{(H,Z)}[max_x p] <= eps`, so the source may have zero entropy on an eps-fraction of oracles. Classical statements quantify per-source and conclude whp over `H`, which cannot be applied here; this is what forces the deviation to be bounded in expectation uniformly over all support sizes at once and then averaged by Jensen. The Dodis--Ostrovsky--Reyzin--Smith average-min-entropy LHL does not patch it either, since there the side information must be independent of the hash key and here `Z` may be `H` itself. The note's closing remark already isolates this correctly. (2) A single universal constant over the whole parameter range including the degenerate corners `D = 1`, `R > D`, `eps = 1`; textbook statements hide `O(1)` in exponents. (3) Nothing in the RO-extraction literature applies, because all of it is query-bounded — Dodis--Guo--Katz compression, Unruh's presampling (CRYPTO'07), Coretti--Dodis--Guo--Steinberger bit-fixing (EUROCRYPT'18), and our own Coretti--Farshim--Harasser--Southern, *Multi-Source Randomness Extraction and Generation in the Random-Oracle Model* (ITC 2025, ePrint 2025/1258), whose abstract names "statistical multi-source extraction with respect to unbounded distinguishers" as an open direction. `c/0004` and `c/0005` are the seeded single-source instance of that direction, and saying so places them properly.

  **A correction, not just an addition.** Remark 5.3 of `proof.tex` calls both matching lower bounds sketches ("For the second we only sketch"). Both are proved theorems: Radhakrishnan and Ta-Shma, *Bounds for Dispersers, Extractors, and Depth-Two Superconcentrators*, SIAM J. Discrete Math. 13(1), 2000, give `d >= log(n-k) + 2log(1/eps) - O(1)` and `d + k - m >= 2log(1/eps) - O(1)` for every `(k,eps)`-extractor, the latter specialising to `k - m >= 2log(1/eps) - O(1)` for strong extractors since `(sd, Ext(x,sd))` is then an ordinary extractor with output `m+d`. Replace the sketch with the citation; it also shows the note's `sqrt(log(D*eps)/K)` shape was the right guess.

  **The work.** Add a related-work subsection to `c/0004/latex/proof.tex` (it currently has none), extend its bibliography beyond McDiarmid, fix Remark 5.3, and add the corresponding sources to `c/0004/index.qmd` and a prior-art paragraph to its Discussion tab. Do the same for `c/0005`, where the payoff is larger: its `difficulty.note` says "the seed's own entropy is the part the existing argument does not already handle", and the extractor dictionary says that part is Vadhan's Theorem 6.14 — so the route to the open conjecture is the same three deltas above applied to the non-strong case, and that should be recorded on the page before anyone spends a proving budget on it. Verify every citation against the source before committing, per the standing rule; the Vadhan theorem numbers and the Radhakrishnan--Ta-Shma formulas above were read out of the PDFs on 28 August 2026 and are quoted correctly as of that date.
