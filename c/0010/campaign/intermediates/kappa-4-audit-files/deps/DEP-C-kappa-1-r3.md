...
Revision r3 of `split-decomp-kappa-1`. r2 repaired four findings upheld in a first
round of blind review; r3 repairs the one finding upheld in a second round — the
class $\kappa^{\mathrm{na}}$ ranges over was underdetermined, because Lemma 0's
proof assumed a per-coin-string reading of challenge resolution that Section 4's
definition did not state — together with four items ruled inconsequential but
cheap. Supersedes both earlier ids; neither earlier tally carries over, each
having been recorded against different content.
...
there is a deterministic such $D'$ with (i) $\mathsf{Adv}_{\mathcal Y,D}\le\mathsf{Adv}_{\mathcal Y,D'}$
and $\bigl|\Pr[\mathsf{Real}=1]-\Pr[\mathsf{Real}_0=1]\bigr|$ no larger for $D$ than for $D'$,
and (ii) $D'$ of the same challenge resolution as $D$. Consequently the suprema defining
...
### END OF ARTIFACT split-decomp-kappa-1-r3 ###
