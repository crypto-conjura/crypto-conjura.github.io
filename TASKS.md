...
- [ ] **Tighten the paper / `c/` LaTeX / Lean statement correspondence (~7h)**

  Requested 23 August 2026: make the conjecture as stated in the original paper, as typeset in `c/<id>/latex/main.tex`, and as formalized in `c/<id>/lean/Statement.lean` correspond as closely as possible. Surveyed before logging; the finding is that the chain has **four** links, not three, and only the non-semantic seams are gated.

  `paper PDF -> SOURCE.md quotes -> c/<id>/latex/main.tex -> index.qmd "## Statement" -> lean/Statement.lean`. CI covers `main.tex` -> PDF (`artifact_manifest.py --check`), the `index.qmd` statement's self-hash (`status_badge.py`, which forces `statement_match` back to `open` on drift), draft-folder existence (`draft_status.py --check`), and that the Lean builds (`lean_action_ci.yml` plus `Audit.lean`'s axiom check). Nothing gates any of the four semantic seams.

  **The unmentioned link is the weak one.** The `## Statement` tab of `c/<id>/index.qmd` and the `conjecture` environment in `c/<id>/latex/main.tex` are independently hand-maintained renderings of the same claim (compare `c/0004/index.qmd`'s `.callout-important` against `c/0004/latex/main.tex:101-110`: identical modulo macro expansion, `\calK` for `\mathcal{K}`, `\Adv^{\extpub}` for `\mathbf{Adv}^{\mathrm{ext\text{-}pub}}`). `statement_sha` hashes only the `.qmd`. Editing `main.tex` and rebuilding its PDF passes every gate while leaving the page saying something else — and the `.tex` is what the page cites as its source of record.

  **Two defects found while surveying, both worth fixing regardless of how much of the rest gets done:**

  - **`statement_sha` is a hash of (content x normalizer version), not of content.** `c/0004/lean/MATCH.md` pins `f1912b71...`; the frontmatter now reads `85c5fd1b...`. The mathematics never changed — both versions normalize identically under today's `statement_text_for_hash` — but the hash basis moved when `_ARTIFACT_ROW_RE` was added to that function in `0556b94`, and `revision` stayed at 1 through it. So any future edit to the normalizer silently detaches every recorded match hash on the site at once.
  - **Nothing reads `MATCH.md`.** It declares its own staleness trigger in prose ("if that hash moves, this table is stale by construction") and is referenced only from `REPORT.md`, the page body, and `Statement.lean`. It is therefore currently stale by its own stated rule while the page still shows `statement_match: ai`. Only `c/0004` has one at all; `c/0001`-`c/0003` have none, consistently with all three sitting at `statement_match: open`.

  **The work, in the order it should be done:**

  - **Generate the conjecture box from the `.tex`, closing seam 3 (~3h, the bulk of the estimate).** Scope it to the claim itself — `\begin{conjecture}...\end{conjecture}` -> the `.callout-important` block — via a macro-expansion table, and leave Setting and the game tables hand-written: those are genuinely different media (an HTML `cj-game-table` against a LaTeX figure) and forcing them through one generator is where the cost stops being worth it. `gen_interface.py` is the in-repo precedent, `.tex` fragment to page box with `--check` in CI. After this the claim has one source and drift is impossible rather than merely detected. Migrating 48 existing pages is the real cost, and it wants per-page eyes, not a bulk rewrite.
  - **Version the hash basis (~0.5h).** Write `statement_sha: "v2:85c5fd..."` and bump the prefix whenever `statement_text_for_hash` changes, so a normalizer edit forces an explicit re-match sweep instead of a silent detach.
  - **Make `MATCH.md` a checked artifact (~0.5h).** YAML frontmatter carrying `statement_sha`, `lean_decl`, `toolchain` and `mathlib_rev`; required whenever `lean.mode != none` and `statement_match != open`; its `statement_sha` gated equal to the page's. This is what would have caught both defects above.
  - **Bind label to declaration, both directions (~1h).** `\label{conj:pub}` in the tex against an `-- informal: c/0004/latex/main.tex#conj:pub` line on the Lean decl, with a script asserting a bijection over published conjecture environments. The hazard this closes is specific: `c/0004` and `c/0005` are variants sharing one source document, differing only in whether the distinguisher receives the seed, and a Lean file formalizing the wrong one of the two would pass everything today.
  - **Promote provenance onto the published object (~1.5h).** The verbatim page-numbered quotes and the source PDF's sha256 sit in `latex/conjectures/<slug>/SOURCE.md` (41 of 50 drafts have one) and never reach `c/`. Copy the statement and openness quotes to `c/<id>/SOURCE.md` and make the mechanical quote match re-runnable against the archived PDF. `c/0049`'s `### Sources` currently carries hand-written pointers ("Remark 1.4 (p. 5)") with no quote and no hash behind them.
  - **Write `prompts/match.md` (~0.5h).** Every other discipline here is driven by a prompt file; the match check is the one that is not, which is why it exists once. Require the discrepancy register under the misformalization taxonomy already adopted in `CONTRIBUTING.md` (Level/Type), and require quantifier order to be called out explicitly — `c/0004`'s own table names it "the one thing in the translation most worth a human's attention," and it is exactly what a sanity lemma cannot catch.
