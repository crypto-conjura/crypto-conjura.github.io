...
- [ ] **Add the supplied three-paragraph roadmap passage to the website — placement and a style question, both worth resolving before pasting (~15min once decided)**

  Requested 19 August 2026 with the exact prose supplied, "somewhere appropriate" left to editorial judgment. Text is not reproduced again here in full — see the request — but in outline: near-term focus on a small vetted corpus over unvetted volume, deferring general archiving, a bifurcated vetted/incubator submission pipeline; a versioned taxonomic model separating statements/attempts/progress/resolutions, AI-provenance categories from "AI-assisted" to "AI-led", and verification methods combining formal checks with human audits; and risk/sustainability — contributor credit by role, licensing (CC BY 4.0 for prose, MIT/Apache-2.0 for code), a responsible-disclosure channel, and a trademark check on the project name.

  **Placement recommendation: `philosophy/index.qmd`.** It's the site's "why this exists" / vision page — the only page whose existing register is this abstract (its own prose already talks about "the economics of problem-solving," "an open, decentralized market for cryptographic solutions," "reviewers are the future authors") — and its closing paragraph already gestures at exactly this kind of forward-looking statement ("August 2026 marks an inflection point... Conjura attempts to bring the cryptology community together to build a new foundation"). Landing the passage as a new section before that closing paragraph, rather than after it, keeps the page's own "we invite researchers to..." call-to-action as the actual ending. No other page is a good fit: `support/index.qmd` (just rewritten, 19 August) is about how to help, not where the project is going; there is no roadmap page.

  **A style question to resolve before pasting, not after — flag this rather than silently rewrite it, since nothing this time asked for smoothing (unlike the two `support/index.qmd` passages earlier the same day, which explicitly did).** Even against Philosophy's own already-elevated register, the supplied passage reads noticeably more like dense consultant/report prose than house style: "systemic integrity necessitates," "combinatorial verification methods," "bifurcated submission pipeline," "remains a paramount structural priority." Every other page on this site — including Philosophy's own existing four paragraphs — favours plainer, more direct sentences even when the content is abstract. Confirm whether the passage should go in verbatim (the request's literal instruction) or whether it should get the same "smoothed, kept close to the original" treatment the two `support/index.qmd` passages got earlier today, before publishing something whose register jars against the rest of the page it sits on.

  **This substantially restates content already tracked, in more concrete and actionable form, across roughly ten items already in this file** (added 19 August 2026 from `maintainer-brief.md`, the same underlying source): the trademark opinion, security/disclosure hardening, governance rules, the research-object/status data model, AI-provenance labels and credit/attempt-report model, the two-lane admission model, and the licensing stack, all in "Credibility and distribution" and "Website and repository" above. Adding the passage to the public site is a communication choice (telling readers where the project is headed) and doesn't replace or shorten any of those tasks — don't let it read as "done" for any of them.
