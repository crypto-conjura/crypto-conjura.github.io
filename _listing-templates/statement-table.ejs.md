<table class="table cj-statement-table list">
<thead>
<tr>
<th class="sortable" data-sort="listing-badge_sigma">Status</th>
<th class="sortable" data-sort="listing-title">Statement</th>
<th class="sortable" data-sort="listing-model">Model</th>
<th class="sortable" data-sort="listing-form">Form</th>
<th class="sortable" data-sort="listing-category">Category</th>
<th class="sortable" data-sort="listing-open_obligations">Open</th>
</tr>
</thead>
<tbody>
<% for (const item of items) { %>
<tr <%= metadataAttrs(item) %>>
<td class="listing-badge_sigma"><span class="cj-status-pair"><a href="<%= item.badge_legend_url %>" class="cj-status-link" aria-label="<%= item.badge_statement_caption %> Click for the status badge legend."><svg class="cj-status-badge cj-status-badge-statement" viewBox="0 0 32 32" width="24" height="24" xmlns="http://www.w3.org/2000/svg" role="img"><% if (item.badge_sealed) { %><circle cx="16" cy="16" r="15" fill="none" stroke-width="1.5" class="cj-seal"/><% } %><circle cx="16" cy="16" r="13" fill="none" stroke-width="4" class="cj-ring-<%= item.badge_sigma %>"<% if (item.badge_dash) { %> stroke-dasharray="<%= item.badge_dash %>"<% } %>/></svg></a><a href="<%= item.badge_legend_url %>" class="cj-status-link" aria-label="<%= item.badge_proof_caption %> Click for the status badge legend."><svg class="cj-status-badge cj-status-badge-proof" viewBox="0 0 32 32" width="24" height="24" xmlns="http://www.w3.org/2000/svg" role="img"><% if (item.badge_sealed) { %><circle cx="16" cy="16" r="15" fill="none" stroke-width="1.5" class="cj-seal"/><% } %><circle cx="16" cy="16" r="8" class="cj-disc-<%= item.badge_pi %>"/><text x="16" y="17" text-anchor="middle" dominant-baseline="central" class="cj-glyph" font-size="9"><%= item.badge_glyph %></text></svg></a></span></td>
<td class="listing-title">
<a href="<%- item.path %>"><%= item.short_title %></a><br/>
<span class="cj-status-summary"><%= item.status_summary %></span>
</td>
<td class="listing-model"><%= item.model %></td>
<td class="listing-form"><%= item.form %></td>
<td class="listing-category"><%= item.category %></td>
<td class="listing-open_obligations"><%= item.open_obligations %></td>
</tr>
<% } %>
</tbody>
</table>
