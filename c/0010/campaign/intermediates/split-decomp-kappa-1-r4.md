# split-decomp-kappa-1-r4 — the extraction bounds and the decomposition chain

**Revision of `split-decomp-kappa-1-r3` after triage of the six class-(C) findings its blind pass
reported and never adjudicated.** That pass returned verdict CLEAN and logged the six "without
forcing the verdict"; no triage file and no revision followed, so they stood as an open debt.
Triage ruled **4 upheld, 2 pedantic, 0 overruled**. All four repairs are here. **No theorem
statement changes, no constant moves, and nothing is reproved.**

- **T1 (upheld).** Lemma 0 asserted a single deterministic $D'$ dominating *both* paired
  quantities, where its proof maximises one quantity over coins and then says the argument is
  identical for the other. Different experiments have different maximising coin strings. Triage
  supplied a witness: deterministic profiles $(1/2,0)$ and $(0,1/2)$ at equal weights give
  mixture $(1/4,1/4)$ and an empty dominator set, so the conjunction is not a consequence of the
  argument given. **Repaired** by fixing one experiment in the statement — which is how
  `split-decomp-kappa-2-r2` had already independently restated it. Both call sites, Theorems A
  and B, use one quantity at a time, so nothing downstream needed the conjunction.
- **T2 (upheld, scoping).** Lemma P Step 2's *vacuity* case split — "if $\delta_\zeta>1$ the
  asserted bound exceeds $q\log M$" — needs $S_\zeta\le\bar S$, which holds only off the bad set
  $\mathcal B$. The density half of Step 2 needs no such thing, $\delta_\zeta$ being built from
  the actual $S_\zeta$. Machine-checked: the inference fails at 29335 grid points, **every one of
  them with $S_\zeta>\bar S$ and none with $S_\zeta\le\bar S$**; smallest witness $\log M=1$,
  $\gamma=1/8$, $\sigma=0$, $\bar S=5$, $P=11$, $q=1$, $S_\zeta=9$, where the bound is $0.9773$
  against $q\log M=1$. **Repaired** by scoping Step 2 to $\zeta\notin\mathcal B$ and displaying
  the inequality that needs it.

  Triage checked the ordering hardest, since a bad-set argument that recovers its own hypothesis
  later would be circular, and found the order **right, not wrong**: $S_\zeta\le\bar S$ off
  $\mathcal B$ comes from Step 1's *definition* of $\mathcal B$, not from Step 4, and Step 4's
  separate charge $\Pr[\mathbf z\in\mathcal B]<\gamma$ covers the bad $\zeta$ whatever family
  Step 2 assigns there. Lemma P's stated bound is reproduced exactly.
- **T4 (upheld).** Section 0's item (C$'$) dropped the per-coin qualifier that item (A) calls
  load-bearing. It is verbatim the in-law phrasing whose ambiguity an earlier round upheld — the
  ruling that created r3 — and Section 4 of this very document exhibits the $N=M=2$, $q=1$
  observer that is challenge-independent in law while each coin fixing has resolution $2$.
  **Repaired** by inserting the qualifier. Theorem C$'$ itself was already unambiguous.
- **T5 (upheld).** Section 0's item (C) wrote the family as $\mathcal Y^{P,\gamma}$ where
  Theorem C proves it for $\mathcal Y^{P,\gamma/2}$. Lemma P at slack $\gamma$ yields $+2\gamma$,
  not $+\gamma$: 159 grid points where item (C)'s display fails to cover Lemma P at slack
  $\gamma$, and none at $\gamma/2$. **Repaired**, one superscript. Purely a statement-level slip
  in an overview section — Theorems C and C$'$ already carry $\gamma/2$, which is what converts
  Lemma P's $2\gamma$ into the Contract's $+\gamma$, and $c=2$, $C=8$ are unaffected at either
  slack.

Ruled **pedantic**, no repair, recorded so they are not raised a third time. Lemma P Step 3
mentions averaging over the challenge but not over $D$'s coins: card S1 records that
[CDGS, Claim 3]'s proof is without loss of generality deterministic, so the bound is uniform in
$\rho$ and the same line already written for the challenge covers it, with Lemma 0 as a second
exit. And Section 10's [CDGS, Claim 2] bullet attributes two conclusions to a card invariant
written for the uniform variable: read with the card's uniform $X$ the invariant is false at
initialisation, so its $X$ must be $X_z$ — the referent an earlier triage settled by this same
argument — and both conclusions follow independently from card point 2 together with Claim 2's own
"$\gamma$-close to", which measures under $X_z$.

**Verification status: one pass, verdict CLEAN, now with its findings adjudicated.** Four
verification angles remain owed against the lineage standard of five, and this revision has had
none of its own. The triage's own answer to whether an errata note would have sufficed: it would
have been defensible, since no theorem statement changes, but the four edits total about five
lines and a revision is cheaper than a standing errata.
...
restricts the supremum to observers whose *query positions*, for each fixed coin string, do
not depend on the challenge value (Section 4 makes this precise, and the coin quantifier is
load-bearing); and $\kappa(0)\le 5\sqrt{\sigma'\delta}$ with no restriction.
...
**(C)** An explicit family $\mathcal Y^{P,\gamma/2}$, chosen before $q$, with
...
$q^{+}\delta\le\sigma'$, provided either the observer's query positions are, *for each fixed coin string*,
independent of the challenge value, or $M\le\sigma'/\sqrt{27\delta}$.
...
**Lemma 0 (derandomisation).** *Fix either one of the two paired experiments.* For every $q$
and every $q$-query challenge-oblivious $D$ there is a deterministic such $D'$ with (i) the
advantage of that experiment no larger for $D$ than for $D'$, and (ii) $D'$ of the same challenge
resolution as $D$. Consequently the suprema defining
$\kappa(q)$ and $\kappa^{\mathrm{na}}(q)$ are both attained on deterministic observers, the
latter on deterministic observers of resolution $1$.
...
inherits $D$'s challenge resolution: the definition in Section 4 quantifies over every coin
string, so if $D^{f}_{\rho}(v,\zeta)$ and $D^{f}_{\rho}(v',\zeta)$ issue the same query
positions for every $\rho$ whenever $h(v)=h(v')$, they do so in particular at $\rho^{*}$,
which is what $D'$ runs. The argument is identical for either paired experiment, which gives
both parts of the statement. $\square$
...
so with $x=1/m$, $1/k\le2m$. If $k=1$ the claimed decomposition is the identity
$\pi=\sum_u\pi(u)\,\delta_u$, a convex combination of distributions uniform on singletons,
and no external result is needed; assume $k\ge2$. Since $\|\pi\|_\infty\le1/k=2^{-\log k}$,
$\log k>0$ and $2^{\log k}=k\in\mathbb N$,
...
Say $D$ has **challenge resolution $M'$** if there is $h:[M]\to[M']$ such that for
**every coin string $\rho$ of $D$**, every $f$, every $\zeta$, and all $v,v'$ with
$h(v)=h(v')$, the runs $D^{f}_{\rho}(v,\zeta)$ and $D^{f}_{\rho}(v',\zeta)$ issue the same
sequence of query *positions*; the output bit is unconstrained. Every $D$ has resolution $M$;
resolution $1$ means that, for each fixed coin string, the query positions ignore the
challenge value. We may take $h$ surjective without loss: replacing $[M']$ by
$\mathrm{image}(h)$ issues fewer runs in Lemma 4 and only shrinks its budget.

The coin quantifier is part of the definition and not a technicality. The weaker condition
that the query positions be challenge-independent *in law* defines a strictly larger class:
at $N=M=2$, $q=1$, the observer that flips $\rho$ and queries $(1,1)$ when $v\oplus\rho=1$
and $(2,2)$ otherwise has query position uniform on $\{(1,1),(2,2)\}$ for both challenge
values, yet each of its two coin fixings has resolution $2$. Such an observer steers its
probes by the challenge once its coins are fixed, which is exactly the behaviour
Section 9's Barrier 1 leaves open, and it is not in the class $\kappa^{\mathrm{na}}$ ranges
over.
...
fixed $v_j\in h^{-1}(j)$ — which exists, $h$ being surjective as Section 4 permits us to
assume — inspecting each queried cell not already known" form a revealing
...
$\zeta\in\{0,1\}^{*}\times\{0,1\}^{*}$ — with $Y_{f,\zeta}$ the uniform distribution on
$\mathsf{Fun}$ at every $\zeta$ outside the support of $\mathbf z$, so that the family is
total as the Contract requires, no experiment reaching such a $\zeta$ — depending only on
$(S_1,S_2,P,\gamma)$, in which
...
**Step 2.** Fix $\zeta\notin\mathcal B$ — for $\zeta\in\mathcal B$ set $Y_{f,\zeta}$ uniform,
Step 4 charging those $\zeta$ against $\Pr[\mathbf z\in\mathcal B]<\gamma$ — and set
$\delta_\zeta:=(S_\zeta+\log\gamma^{-1})/(P\log M)$. If $\delta_\zeta>1$ then
$P\log M<S_\zeta+\log\gamma^{-1}\le\sigma+2+2\log\gamma^{-1}$, the last step using
$S_\zeta\le\bar S$, which holds because $\zeta\notin\mathcal B$; so the asserted bound exceeds
$q\log M\ge q$, hence is vacuous for $q\ge1$,
...
the window left by Corollaries B$'$ and Section 8. Note that an observer whose query
positions are challenge-independent only *in law*, and which steers its probes by the
challenge once its coins are fixed, has per-coin resolution $M'>1$ and therefore sits here
rather than in Theorem C$'$'s first branch; the coin quantifier in Section 4's definition is
what places it here. This is not slack in the counting.
...
- **[CDGS, Claim 2]** (Coretti, Dodis, Guo, Steinberger, *Random Oracles and Non-Uniformity*,
  ...
  components' supports and the fact that fixed values are values the sampled $f$ takes; and,
  via the recursion invariant $\Pr[Y=y]=\Pr[X=y\mid X\in\mathrm{supp}(Y)]$ recorded on the
  card, that the decomposition is of $X_z$ itself — so each $\lambda_jX_j$ is the
  restriction of $X_z$ to $\mathrm{supp}\,X_j$ and the residue carries $X_z$-mass at most
  $\gamma$, both measured under $X_z$ and not under the uniform distribution. All three
...
### END OF ARTIFACT split-decomp-kappa-1-r4 ###
