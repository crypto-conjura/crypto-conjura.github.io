...
The whole difference is the **selection** term, and there the LHL square-roots it. To match $\tfrac65\sqrt{a/K}$ with $a = 1+\ln D$ one would need $\gamma \lesssim a/K$. But $\bar\beta$ is a collision frequency read off the $Kt$ table entries above the source's support, and the source picks that support after seeing $H$, so the estimate must hold uniformly over $\binom{D}{t} \approx e^{ta}$ of them. A mean of $Kt$ bounded terms, uniform over $e^{ta}$ events, cannot be pinned closer than $\sqrt{ta/(Kt)} = \sqrt{a/K}$; so $\gamma = \Omega(\sqrt{a/K})$ and $\tfrac12\sqrt\gamma = \Omega\bigl((a/K)^{1/4}\bigr)$. **A fourth root against a square root, forced by the $\tfrac12\sqrt{\cdot}$ in the leftover hash lemma itself** rather than by any choice of concentration inequality. In seed length that is $d = \log n + 4\log(1/\delta)$ against the proved $\log n + 2\log(1/\delta)$: an extra $2\log(1/\delta)$ bits, and the [Radhakrishnan--Ta-Shma optimum](https://doi.org/10.1137/S0895480197329508) is the latter.

Certifying $\gamma$ with the same bounded-differences argument the proof uses is cruder still -- it yields only $\gamma \le R\sqrt{2a/K}$, hence $\tfrac12\sqrt{R}\,(2a/K)^{1/4}$ and $2\log R$ further bits of seed -- but that $\sqrt R$ is an artefact of the certification and a variance-aware one removes it. The fourth root is what survives, and it is why the optimal-extractor proofs work in $L_1$ instead, by concentration plus a union bound over flat sources. The [c/0004](/c/0004/) proof takes that route, arriving at it independently.
