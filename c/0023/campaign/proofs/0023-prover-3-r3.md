...
# An internal barrier for rung R2 (I02): **two** window-payment sub-classes — relevance-denominated, and own-heavy *while nonempty at the codimension-$d$ subcube $C$* — cannot beat $\mathrm{poly}(d)\,2^{-d}$; the class as a whole is **not** capped, and the two live plans escape both caps: P1 outright, P5 for minimum-size certificate selections (its declared minimal-certificate variant is open, [G5])

**r3 (this file) is a bounded, TEXT-ONLY repair of `0023-prover-3-r2` against
the triage ruling `0023-prover-3-r2-triage.md`** (5 upheld findings U-A1, U-A2,
U-B1, U-B2, U-C1; 4 pedantic P-a–P-d). **No theorem, constant, proof, witness
table or gap was changed** — every disputed number was re-derived in exact
arithmetic during that triage and all of them match; the repair is to the title,
to four VERDICT clauses, to §5.3's displayed record sentence, to Remark 1.1,
Remark 2.1, Remark 4.3 and two register rows. The r3 changelog is at the end,
after the r2 changelog, which is retained for the record. Everything else below
is r2 verbatim.

r2 was in turn a revision of `0023-prover-3` against the triage ruling
`0023-prover-3-triage.md`
...
1. **CLAUSE (iv) FIRST, since it is the campaign-critical finding: P1 and P5
   ESCAPE both caps — YES, both, by exact computation: P1 outright, P5 for
   minimum-size certificate selections ([G5] for its declared
   minimal-certificate variant, which is open).** The strategist's ruling
   ...
   Precisely what is proved (§7): **neither cap yields any bound below
   $1/\mathrm{poly}(d)$ for the shattering window (P1), for the
   minimum-certificate window (P5), or for $W_{\max}$.** For **CAP I** this is
   *not* a failure of its hypothesis — T6's nonemptiness hypothesis **holds** for
   all three at the address pair — but a degradation of its conclusion: their
   density there is only $d/(2^{d-1}+d-1)$, so T6(b)'s ceiling degrades to
   $\approx\frac{d+1}{4d}$, vacuous against the $\frac1{2d}$ grid ceiling. For
   **CAP II** the hypothesis genuinely **fails**: own-heaviness fails at every
   level $>\frac1{2(2^d-1)}$, witness (b)'s $D$ attaining that level exactly.
   **Each certified witness therefore contributes no cap below an explicit
   inverse-polynomial number** — so neither cap, and neither witness, caps them
   below $1/\mathrm{poly}(d)$. **No lower bound on
...
3. **SCOPE (proved), with the ladder consequence stated exactly.** Both
   ...
   below — **this displayed sentence, and only this one, is the sentence intended
   for the campaign's ladder record (`LEDGER.md`, `PROGRESS.md`); the r2 version
   of it must not be pasted there, because it omitted CAP II's nonemptiness
   hypothesis**:
   > *No relevance-denominated window-payment argument, and no window-payment
   > argument whose window is own-heavy at some level
   > $\theta>\frac1{2(2^d-1)}$ **and nonempty at the codimension-$d$ subcube
   > $C$**, can establish R4's target $c\,e^{-d^\alpha}$ — or any threshold
   > above $(d+1)2^{-d-1}$ resp. $\frac1{2(2^d-1)}$ — in the object class of
   > R2, R3, R4, R5, R6′ or R6($\mathbb Z_2$).*
...
5. **FRONTIER (proved, and it is the artifact's most reusable output).** Every
   ...
   $\eta^*(d)$ — which, by §8.1's list, includes $W_{\mathrm{rel}}$,
   $W_{\mathrm{sh}}$, $W_{\max}$ and the certificate windows, hence both live
   plans'; it does **not** include $W_{\mathrm{Forced}}$ or
   $W^\theta_{\mathrm{hvy}}$, which are empty at some pairs of $\mathcal P_d$ and
   for which only T3's restricted ceiling $\eta^*_W$ is claimed — is capped by
   the single number
...
*Remark 1.1$'$ (the two roles of a witness pair, kept distinct; pedantic item
P-a of the r2 triage).* A witness pair is used in this artifact only **(i)** to
instantiate (T2)'s universal quantifier over $\mathcal P_d$, pinning $\rho$ at
one window-size profile, and **(ii)** to certify, through the point masses at
its two sides, that that profile is realised by some incompatible family —
**never** as a counterexample to the conclusion of an argument in $\mathcal W$.
Scope in the sense of Remark 1.1 is a property of a *family* offered as a
counterexample; a *pair* carries no $\delta$ and no threshold, so scope is not
even a predicate that applies to it. The $\max\delta$ values quoted above are
therefore irrelevant to T2 and to both caps, and no cap here is conditional on
any scope convention.
...
**pair** breaks T1 and is outside $\mathcal W$. (That observation — that
pair-dependence is what obstructs the per-function factorisation — is **this
artifact's own**, not card S7's. Card S7,
`sources/S7-changfang26-card.md`, records its *obstacle (ii)* as the
**non-canonicity of the maximum-degree window $T(A)$ across a distribution's
support, so that the union $T$ over a whole family is not bounded and the
argument must be run pairwise or with a covering step**; that is the sense in
which §7.2's *Reading* invokes it. Canonicity is *not* required for T1.)
...
for every such rule with $\theta\le\frac12$ — which is the whole of the
interesting range, since $\mathrm{Inf}_i(f_A)\le\frac12$ for every nonempty $A$
and every $i$ (write $P_1=\Pr_x[\text{exactly one of }x^{i\to+1},x^{i\to-1}\in
A]$ and $P_2=\Pr_x[\text{both}\in A]$; then $\alpha_A=P_2+\frac{P_1}2$ and, as in
T4(c), $\mathrm{Inf}_i(\mathbf 1_A)=\mathbb E[D_i^2]=\frac{P_1}4$, so
$\mathrm{Inf}_i(f_A)=\frac{P_1/4}{P_2+P_1/2}\le\frac12$), so at
$\theta>\frac12$ the rule
$W^\theta_{\mathrm{hvy}}$ is identically empty on the class, witness (b)
supplies no bound, and what may be said there turns on D5's degenerate clause,
**[ESCALATED E1]**; the cap asserted here is for $\theta\le\frac12$
(pedantic item P-b of the r2 triage).
...
Therefore:
> **No relevance-denominated window-payment argument, and no window-payment
> argument whose window is own-heavy at some level $\theta>\frac1{2(2^d-1)}$
> _and nonempty at the codimension-$d$ subcube $C$_, can establish R4's target
> $c\,e^{-d^\alpha}$ — or any threshold above $(d+1)2^{-d-1}$ resp.
> $\frac1{2(2^d-1)}$ — in the object class of R2, R3, R4, R5, R6′ or
> R6($\mathbb Z_2$).**

**This display is the sentence intended for the campaign's ladder record.** Its
second clause carries **both** hypotheses of T8 (CAP II): own-heaviness at a
level $\theta>\frac1{2(2^d-1)}$ **and** $W(C)\ne\emptyset$. The omission of the
second in r2 was not vacuous: own-heaviness (D2) is an inclusion constraint, not
a selection rule, so a localised $W$ may be own-heavy at $\theta=\frac14$, have
$W(C)=\emptyset$, and take the address hub $W(A_k)=\{a_1,\dots,a_k\}$ on witness
(a), for which T6 yields only $\frac{d+1}{4(d-1)}$ — no exponential cap at all.
Such a $W$ is capped by nothing in this artifact (Remark 4.2$'$).
...
at witness (b)), and $\eta^*(d)\in[2^{-d},1]$ is open (§8.2; see the r3 note at
the end of §8). The ladder record must carry the displayed conditional sentence
**exactly as displayed, including its nonemptiness clause** — not a summary of
it, and not r2's version of it.
...
*Note (added in r3; record-keeping only — nothing in §§1–8 is retracted, and no
claim above is conditioned on it).* Two facts established after this artifact was
drafted, recorded so that a cold start is not mis-steered.
**(1) Item 2's open bracket and item 5's milestone are SUPERSEDED, not
falsified.** `proofs/0023-refuter-5.md` (CERTIFIED, not verified) proves that the
relevance-payment floor is exactly $1$, i.e. $\eta^*(d)=1$ for all $d$; the r2
triage independently searched all cross-disjoint degree-$\le d$ pairs at
$(N,d)=(3,2),(4,2),(4,3)$ and found $\min\pi_{\mathrm{Rel}}=1$ exactly, attained
with $|S|=1$. So §8.5's milestone is resolved **in the negative** (no pair with
$\pi_{\mathrm{Rel}}<1$), and the bracket of item 2 — also called open in
VERDICT 1 and VERDICT 5 — is closed at its upper end by that later artifact.
Items 2 and 5 were honest at the time of writing and remain correct as
statements about what *this* artifact determines; the ledger must record
$\eta^*=1$ (with refuter-5's CERTIFIED status, or after a verification pass —
the human's call) rather than carry "$\eta^*$ bracket open" forward.
**(2) The two concrete killers this artifact generalises stand
unconditionally.** Each of `0023-refuter-3` §4's two certificates refutes a
*universally quantified per-pair inequality* by one explicit pair — the address
pair against $\pi\ge(|J|+|K|)/(2d)$, and $(C,C^c)$ against (HEAVY$_\theta$) at
every $\theta\ge1/\mathrm{poly}(d)$ — and a per-pair inequality carries no
family, no $\delta$, no threshold and hence no notion of scope. Neither killer
depends on D4's class model, on D5's degenerate convention (**[ESCALATED E1]**),
or on anything else stipulated here: they would survive the complete demolition
of this artifact's class model.
...
| **S7** (obstacle (ii)) | `sources/S7-changfang26-card.md` | obstacle (ii) as the card states it: *non-canonicity of the maximum-degree window $T(A)$ across a distribution's support, so the union $T$ over a whole family is not bounded and the argument must be run pairwise or with a covering step*. The card says nothing about influences, families, pairs or factorisation; the observation that a **pair-dependent** window is what obstructs the per-function factorisation is **this artifact's own** (Remark 2.1), not the card's | CARD, READ | §7.2 *Reading* (the card's sense); Remark 2.1 cites the card only to contrast it | no (framing) |
...
| — | `proofs/0023-refuter-2.md` §2 | the master count (M), $\pi(A,B)\ge(|J|+|K|)/(2d)$ for cross-disjoint pairs, transplanted to $\mathcal C^{\mathrm{ind}}_d$ | CERTIFIED (not verified) | Corollary 3.2 (named only, as the inequality CAP I caps) | no |
| — | `proofs/0023-prover-3-u5.md` (L11) | minimal-certificate coordinates are relevant (one-line proof) | OWN UNIT FILE, READ | [G4]; localisation check in §7.4 | no (also evident from the explicit certificates) |
...
## CHANGELOG, r2 (each finding of `0023-prover-3-triage.md` → what changed)

*Retained here unaltered, for the record; the r3 changelog follows it.*
...
## CHANGELOG, r3 (each upheld finding of `0023-prover-3-r2-triage.md` → the exact edit)

**Scope of this revision, stated first: no theorem, no constant, no proof, no
witness table, and no gap was touched.** T1–T9, CAP I's and CAP II's constants,
both witness influence tables, §5.4's group formulas, §6.4's comparisons,
§7.1–§7.5's ratios and the certificate arithmetic, and [G1]–[G5] stand exactly
as in r2 — the r2 triage re-derived every disputed number in exact arithmetic
and all of them match. The caps are **not** restated as scope-conditional: the
triage OVERRULED the class-B objection that proposed it (that objection's model
collapses "provable by the class" into "true" and would credit frozen R1 with a
constant its proof does not deliver), and such a restatement would be a
weakening in response to a mistaken objection. Status remains **PARTIAL** and
every declared gap is preserved verbatim.

**U-A1 (class A — CAP II's nonemptiness hypothesis dropped in three places).**
Restored "**and nonempty at the codimension-$d$ subcube $C$**" in all three:
(i) the **title**, whose second sub-class now reads "own-heavy *while nonempty at
the codimension-$d$ subcube $C$*"; (ii) **§5.3's display**, replaced by the
triage's corrected sentence verbatim, now followed by two sentences recording
that the display **is** the sentence intended for the ladder record and that the
omission was non-vacuous (an own-heavy $W$ at $\theta=\frac14$ with
$W(C)=\emptyset$ taking the address hub gets only $\frac{d+1}{4(d-1)}$ from T6);
(iii) **VERDICT 3's copy** of the display, with the same corrected wording and an
explicit instruction that r2's version must **not** be pasted into
`LEDGER.md`/`PROGRESS.md`. §5.3's closing instruction now says "exactly as
displayed, including its nonemptiness clause". T8 itself already carried both
hypotheses and is unchanged; so are VERDICT 2(ii) and Remark 4.2$'$.

**U-A2 (class A — P5 cleared unqualified although §7.4 evaluates MINIMUM-size
certificates while P5's declared notion is MINIMAL).** Qualified in both flagged
places: the **title** now ends "…escape both caps: P1 outright, P5 for
minimum-size certificate selections (its declared minimal-certificate variant is
open, [G5])"; **VERDICT 1's first sentence** now reads "…by exact computation: P1
outright, P5 for minimum-size certificate selections ([G5] for its declared
minimal-certificate variant, which is open)". **P1 is deliberately not
qualified**: §7.1 evaluates exactly P1's declared window and the ratio is
$\frac14,\frac5{24},\frac{13}{64}$ for *every* choice of the two top supports
(re-verified exhaustively in the triage). §7.4, §7.5 and [G5] already carried
the minimum/minimal distinction and are unchanged.

**U-B1 (class C — VERDICT 5's inclusion claim unproved for two named rules).**
VERDICT 5's parenthetical no longer says "includes every functional named in this
artifact"; it now names §8.1's restricted list ($W_{\mathrm{rel}}$,
$W_{\mathrm{sh}}$, $W_{\max}$, the certificate windows, hence both live plans')
and states explicitly that $W_{\mathrm{Forced}}$ and $W^\theta_{\mathrm{hvy}}$
are **not** covered, being empty at some pairs of $\mathcal P_d$, for which only
T3's restricted ceiling $\eta^*_W$ is claimed. §8.1 is unchanged.

**U-B2 (class C — "the hypotheses of CAP I … fail" is literally false).**
VERDICT 1's "what is proved" sentence is rewritten: CAP I's nonemptiness
hypothesis **holds** for all three functionals at the address pair, and what
happens is that T6(b)'s ceiling *degrades* to $\approx\frac{d+1}{4d}$ (their
density there being $d/(2^{d-1}+d-1)$), vacuous against the $\frac1{2d}$ grid
ceiling — so neither cap yields any bound below $1/\mathrm{poly}(d)$. The CAP II
half is left as it stood, being accurate: own-heaviness genuinely fails at every
level $>\frac1{2(2^d-1)}$, witness (b)'s $D$ attaining the level exactly. §7.1's
body already said this correctly and is unchanged.

**U-C1 (class D, light — the S7 obstacle-(ii) row misdescribes the card).** The
register row now states obstacle (ii) in the card's own words (non-canonicity of
$T(A)$ across a distribution's support, unbounded union over a family, hence
pairwise or covering), records that the card says nothing about influences,
families, pairs or factorisation, and attributes the pair-dependence /
factorisation observation to **this artifact**. **Remark 2.1** was reworded to
match: the pair-dependence observation is marked as this artifact's own, the
card's obstacle (ii) is quoted in its own sense, and §7.2's *Reading* — which
uses the card correctly — is pointed at. The row remains non-load-bearing.

**PEDANTIC items folded in (no diff beyond the lines already touched).**
**P-a**: new Remark 1.1$'$ distinguishing the two roles of a witness pair
(instantiating (T2)'s quantifier over $\mathcal P_d$; certifying that a
window-size profile is realisable) from the role Remark 1.1 speaks of (a family
offered as a counterexample), and stating that scope is not a predicate of a
pair. **P-b**: Remark 4.3 now notes that $\mathrm{Inf}_i(f_A)\le\frac12$ always,
so at $\theta>\frac12$ the rule $W^\theta_{\mathrm{hvy}}$ is identically empty,
witness (b) supplies no bound, and the cap asserted there is for
$\theta\le\frac12$; what holds above $\frac12$ turns on D5's degenerate clause,
which stays **[ESCALATED E1]** — the reviser does not choose it. **P-c**: two
register rows added (`0023-refuter-2` §2's master count (M);
`0023-prover-3-u5` L11), both non-load-bearing. **P-d**: no edit — the triage
ruled §6.1's "T9 gives $\ge\frac14$ there" correct as stated.

**Record-keeping note added at the end of §8** (not a finding): item 2's open
bracket and item 5's milestone are **superseded, not falsified**, by
`0023-refuter-5` (CERTIFIED, not verified), which proves the relevance-payment
floor is exactly $1$, i.e. $\eta^*(d)=1$; and the two concrete killers this
artifact generalises stand **unconditionally**, each refuting a universally
quantified per-pair inequality by one explicit pair, independent of any class
model. Nothing above is retracted and no claim is conditioned on either fact.

**Not changed, deliberately.** Every [ESCALATED] item stays where it is (E1, and
the E2/E3 items the triage raises are answered only by the wording above, not by
a choice of convention). No [SOURCE-BLOCKED] marker exists in this artifact, so
none was moved or discharged; the SOURCE REQUEST remains "none". The two
[MEMORY] items remain flagged and unused.

### END OF ARTIFACT 0023-prover-3-r3 ###
