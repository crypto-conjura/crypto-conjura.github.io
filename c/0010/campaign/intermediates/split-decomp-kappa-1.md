...
Fix $N,M\in\mathbb N$ with $N\ge 2$ and let $\mathsf{Fun}:=\{f:[N]\times[N]\to[M]\}$, with
...
$D':=D_{\rho^{*}}$, which is $q$-query and challenge-oblivious. $\square$
...
away, only $|\mathcal Z|<2^{\sigma+2}$ of them are reached. Setting $k_1=1$ degenerates the
bound to $t\ge\sqrt L\gg1$: the lemma is vacuous for one source, as it must be.
...
*Proof.* If $\delta=1$ then $\sqrt{\sigma'\delta}\ge\sqrt2>1$ and there is nothing to prove;
...
*Proof.* Proposition 6.2 with $M'=M$ and $\gamma_0=\delta$, as in Theorem A. $\square$
...
**Lemma P.** Let $N\ge2$, $P\in\mathbb N$, $\gamma\in(0,1)$. There is a family
...
challenge-oblivious $D$,
$$\mathsf{Adv}_{\mathcal Y,D}\ \le\ \kappa(q)+\frac{2(\sigma'+\log\gamma^{-1})q}{P}+\gamma+P\delta+q\delta.$$

*Proof.* Interpolate $\mathsf G_0=\mathsf{Real}$, $\mathsf G_1=\mathsf{Real}_0$,
$\mathsf G_2=\mathsf{Dec}_0$, $\mathsf G_3=\mathsf{Dec}$.

$|\mathsf G_0-\mathsf G_1|\le\kappa(q)$ by definition of $\kappa$.
...
*Proof.* $\kappa(q)\le6\sqrt{\sigma'q^{+}\delta}$ by Corollary A$'$ or B$'$;
...
### END OF ARTIFACT split-decomp-kappa-1 ###
