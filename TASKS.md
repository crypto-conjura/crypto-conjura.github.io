...
- [ ] **Render `c/0004`'s Lean files as a syntax-highlighted HTML page (~1.5h)**

  Requested 31 August 2026: "use some doc generation tool to generate an html page for the lean file for 0004", with syntax highlighting. Today `c/0004/index.qmd:60` links the formal statement as a raw `lean/Statement.lean`, which the live site serves as unstyled plain text (Quarto copies it verbatim into `_site/c/0004/lean/`), and the same is true of every other campaign's `Statement.lean`. The package is four files — `Statement.lean` (243 lines, 16 declarations), `Proof.lean` (476, 31), `Audit.lean` (21) and `AuditProof.lean` (27) — all four tracked, `.lake/` gitignored as ~7 GB of Mathlib.

  **Tool survey, done 31 August 2026 before logging this, because the obvious answer does not work.**

  - **doc-gen4** is the canonical Lean documentation generator and is version-current (its `main` pins `leanprover/lean4:v4.34.0-rc2`, so a ref compatible with this repo's `v4.33.0` exists). It is nonetheless the wrong tool here: `lake build <lib>:docs` needs the `docs` facet of every transitive import, and a BFS over the module graph from the five Mathlib modules these files import reaches **8,566 modules, ~8,300 of them Mathlib** — essentially all of it. That is the full Mathlib docs build, hours of compute and several GB of HTML, to document 767 lines. Note also that `.github/workflows/lean_action_ci.yml` already carries a comment recording why `leanprover-community/docgen-action@v1` is deliberately not wired in (its unconditional Pages deploy would race `publish.yml` and could overwrite the live site) and names the precondition for re-adding it: "once Lean docs have a deliberate home (e.g. a subpath the Quarto build copies in)". This task is that home, so update that comment if it lands.
  - **LeanInk + Alectryon** — the combination that gives hover-over goal states, which is what would actually make a proof file readable in a browser — is dead for this toolchain. LeanInk's last commit is 18 July 2024 and its `lean-toolchain` reads `v4.6.0-rc1`, twenty-seven minor versions behind `v4.33.0`; it would need porting before it ran at all.
  - **Quarto's own highlighter cannot do it**, which is why a generated page is needed rather than a ```` ```lean ```` fence in a `.qmd`: `quarto pandoc --list-highlight-languages` lists 163 languages and none of them is Lean.
  - **Pygments is the tool that works.** Version 2.21 ships a real `Lean4Lexer`; tokenizing `Statement.lean` with it gives 2,361 tokens and **zero `Error` tokens**, so the grammar handles this file's Unicode, `PMF`, anonymous constructors and docstrings without falling over. Pygments is not installed in this environment (`python3 -m venv` + `pip install pygments` works; same pattern as the PyYAML note above).

  **Shape to build**, following the `scripts/build_uc_html.sh` precedent rather than inventing a second convention: a `scripts/build_lean_html.py` that takes a package directory (not hardcoded to `0004` — `c/0001`–`c/0003` each have a `Statement.lean` and `c/0023/campaign/lean/` a prover skeleton, all of which want the same treatment) and writes one standalone page per package to `c/<id>/lean/html/index.html`, with: all files of the package in `Statement`, `Proof`, `Audit` order; `id="L<n>"` line anchors so a page can be cited by line the way the repo already cites `file:line`; a declaration index built from the `theorem`/`lemma`/`def`/`instance` lines; docstrings and `/-! -/` module comments rendered as prose rather than dimmed comment text; and every `sorry` visibly marked, since `Statement.lean`'s single `sorry` is the whole point of the file. Palette, serif faces and link colours restated as plain CSS copied from `theme-light.scss`/`theme-dark.scss` with the same `:not([data-theme=…])` dark-mode guard, exactly as the UC edition does and with the same caveat that nothing detects drift between them.

  **Wiring.** Add `"c/*/lean/html/**"` to `_quarto.yml`'s `resources` (the UC edition's precedent; Quarto does copy stray project files, but be explicit) and confirm the 7 GB `.lake/` still does not land in `_site` — it does not today, because Quarto skips dot-directories. Then repoint `c/0004/index.qmd:60` at the HTML page while keeping the raw `.lean` link beside it, for readers who want the file the compiler sees. That edit needs no `status_badge.py` regeneration: `statement_text_for_hash` strips the artifact row before hashing, so `statement_sha` is unaffected and no spurious `revision` bump or `statement_match: open` reset follows.
...
- [ ] **Attack the space lower bound for 3-collisions in the restricted model the site already states (~6h — placed here, out of difficulty order, because it shares its subject with the `c/0003` item above and is blocked on it)**

  Asked 30 August 2026: is there any hope of proving *necessity of space* for 3-collisions in some restricted but meaningful model of computation? The repository already answers the first half. One such model is on the site, and in it a memory-sensitive lower bound is not a hope but a theorem.

  **Where it stands.** [`c/0007`](/c/0007/) states it: repetition-free **oblivious** branching programs in Borodin and Cook's model (SIAM J. Comput. 11(2), 1982), specialised to a random-function oracle `f : [N] -> [N]`, state capped at `S` bits between queries, query schedule fixed in advance. There the bound `T * (S + log_2 N) = Omega~(N)` is proved, via Dinur's multi-pass streaming switching lemma (EUROCRYPT 2020, ePrint 2020/229) applied to the answer stream. The page says plainly that this is *the only lower bound known at `k = 3` that mentions memory at all*, and that for **adaptive** programs nothing memory-sensitive is known. It is tight only at `S` polylogarithmic. What is open is the factor of `sqrt(S)`: the conjecture asks for `T = Omega~(max{N^(2/3), N/sqrt(S)})`, matching a table-and-hunt algorithm, and the page's own difficulty note says both ends are in hand and what remains is closing a constant-shaped gap rather than finding a technique. That is the most attackable memory lower bound for `k = 3` in the file.

  **Three things to do, in order.**

  1. *Close the `sqrt(S)` gap in `c/0007`.* This is the actual research. Run it as a harness campaign against the oblivious statement as printed, refuter in parallel from the start — the upper bound is explicit, so a counterexample to the conjectured curve is a concrete object and worth hunting before spending a proving budget.

  2. *Decide what the restriction is buying, and whether it can be weakened.* Two hypotheses are doing work and they are not the same: obliviousness (fixed schedule) and repetition-freeness (no point queried twice). Say which one the switching-lemma argument actually needs, and whether the standard relaxation — adaptive between blocks, oblivious within them — survives it. A partial answer here is worth more than the `sqrt(S)` factor, because it is the only route to the adaptive case, where nothing is known.

  3. *Wire up the relations.* `c/0007` carries `relations: []`. It, [`c/0003`](/c/0003/) (`k`-collisions under auxiliary input, open for `k >= 3`) and [`c/0050`](/c/0050/) (the Merkle-Damgård STB conjecture) are three cuts at one underlying question in three different models — oblivious sequential, preprocessing, and salted short-collision — and the site does not say so anywhere. At minimum give `c/0007` an edge to `c/0003` and re-run `scripts/check_relations.py`.

  **Blocked on the item above.** Do not compare the models until `c/0003` carries its salt: as printed it is refuted in one line, so any statement relating the oblivious curve to the preprocessing curve would be relating something true to something false.
...
- [ ] **Carry the `c/0004` prior-art finding into the LaTeX note, not just the pages (~2h)**

  The website half landed 28 August 2026: the problem hub `p/leftover-hash-lemma-extraction/` gained a "Relation to the extractor literature" section, and `c/0004` and `c/0005` gained sources, a Discussion subsection and a corrected `difficulty.note`. `latex/proof.tex` was deliberately left untouched, and it is now the only artifact on the site that still reads as though the result had no precedent -- it cites exactly one work (McDiarmid) and has no related-work section at all.

  **The finding, in one paragraph, so this entry stands alone.** Put `n = log_2 D`, `k = log_2(1/eps)`, `m = log_2 R`, `d = log_2 K`. Balancing the summands of the public-seed bound against a target `delta` gives `m = k - 2log(1/delta)` and `d = log n + 2log(1/delta)`: Theorem 6.17 of Vadhan, *Pseudorandomness* (chapter 6, Randomness Extractors) -- a random function is whp a strong extractor with optimal entropy loss. `eq:main-sharp` carries `ln(2eD*eps) ~ (n-k)ln2` and so delivers the sharp `d = log(n-k) + 2log(1/delta) + O(1)`; its `b_t = (1/t)ln C(D,t)` is the textbook count of flat k-sources. The secret-seed conjecture (`c/0005`) is Theorem 6.14, the non-strong case. Radhakrishnan and Ta-Shma (SIAM J. Discrete Math. 13(1), pp. 2-24, 2000) proved both parameter sets optimal. The LHL itself cannot apply: universal families need `K >= D^Omega(1)` and the regime here is `K ~ log_2 D`; the almost-universal repair loses a factor `R` inside the square root, turning `sqrt(log D/K)` into `(R log D/K)^(1/4)`. What is genuinely without precedent is that unpredictability is averaged over `H`, so the source may carry zero entropy on an eps-fraction of oracles -- classical statements quantify per source and conclude whp over the extractor, and the Dodis--Ostrovsky--Reyzin--Smith average-min-entropy LHL does not patch it, since there the side information must be independent of the hash key and here `z` may be `H` itself.

  **The three edits.** (a) A related-work subsection in Section 1 or Section 5 recording the correspondence and, more importantly, the one hypothesis that has no precedent -- the note's own closing remark already isolates it correctly without knowing that is what it had found. (b) Remark 5.3 currently offers matching lower bounds for both summands as sketches ("For the second we only sketch"); Radhakrishnan and Ta-Shma proved both, `d >= log(n-k) + 2log(1/eps) - O(1)` and `d + k - m >= 2log(1/eps) - O(1)`, the latter specialising to `k - m >= 2log(1/eps) - O(1)` for strong extractors since `(sd, Ext(x,sd))` is then an ordinary extractor with output `m+d`. Replace the sketch with the citation and keep only the constant-comparison part, which is genuinely the note's own. (c) Extend `conjurabibliography` beyond McDiarmid to the four sources now listed on the page.

  **The gate.** `scripts/artifact_manifest.py --check` fails in CI when a committed PDF no longer matches the LaTeX it was built from, so this task includes rebuilding `c/0004/pdf/proof.pdf` and confirming the manifest check passes. TeX Live 20260301 is installed locally, and `proof.tex` builds against `conjura-solution.cls` in the same folder.

  **Two judgement calls, deliberately left open.** The note carries a dated AI disclosure ("14 August 2026") and "Reviewed by: Nobody" -- decide whether adding prior art is a revision of that artifact (bump the date, keep the disclosure honest) or warrants a new revision number on the page. And `c/0004`'s `difficulty.reach` is still `adaptation`; that is listed as an open obligation on the page and should be settled in the same pass rather than left to drift.
...
- [ ] **Run the harvester over `latex/harvest/` and promote what survives (~size unknown — this is the largest item in the file; batch it, don't run it all at once)**
  ...
  - **Progress, 28 August 2026: the 148 loose PDFs at the top level were run.** (The batch before this one, which produced [c/0097](/c/0097/) through [c/0101](/c/0101/) from 65 new papers earlier the same day, was committed as `8e99bb3` but never written up here; its own commit message is the record. The batch ordinals in this entry's predecessors are therefore one behind the actual count.) 43 were byte-identical to papers already in `processed/` and went to `pruned/` rather than into `processed/` a second time, which is what the "stop the harvester re-reading papers" item above says the script should do and still does not; 105 were new, 4,409 pages between them. In-session again, sixth time, same reason — `pick_backend()` still has nothing to return. **Eight yielded a statement, published as [c/0102](/c/0102/) through [c/0109](/c/0109/); 97 rejections are recorded per-document in the ledger with the sentence that was passed over and why.** 55 quotes ground against the `pdftotext` layer, **55 `exact`, 0 `near`, 0 ungrounded**, with five claimed page numbers corrected by the grounder (two front-matter offsets in 2022/869, Remark 18 and a hardness remark one page earlier in 2023/845, an ancillae remark one page earlier in 2023/543). 0 chktex and 0 lacheck across all eight, every suppression documented hit-by-hit in its folder's `.chktexrc`. Seven new problem hubs, one existing hub extended (`randomness-complexity-of-mpc` gains a second axis), six new topic slugs (100 total). The ledger now holds **434 documents against 434 PDFs in `processed/`**, no orphans and no hash mismatches in either direction; `pruned/` holds 54.

    **The batch invalidated itself once, and the forward check earned its keep three times.** Agrikola, Couteau and Maier's Conjecture 1 (2021/1341, anonymous transfer with anonymity $1-1/c$) is *resolved by another paper in the same batch* — Quach, Tyner and Wichs, 2023/1483, whose introduction says its main theorem is "solving a conjecture explicitly stated in [ACM22]". Read the batch before harvesting from it. Two more were declined on prior art rather than on the text: the 3-XOR hardness conjecture of 2021/373 sits against Dinur–Keller–Klein (JACM 2024), which proves the known dense-3-XOR algorithms essentially optimal *conditionally on* the sparse k-XOR conjecture — later, conditional, and a threshold statement rather than the advantage bound the conjecture asserts, so adjudicating what it leaves open needs a human, exactly as with Conjecture 4 of 2026/664 last batch. And 2025/190's candidate-hash conjecture lost its motivation to 2025/1877, by two of the same authors, also in this batch.

    **Three editorial calls worth recording.** [c/0108](/c/0108/) is a candidate-hardness conjecture, the second published after [c/0098](/c/0098/) and on the same reasoning: the object is a distribution over strings rather than the paper's own construction, the analysis is theorems rather than assertion (implied by the permuted puzzles conjecture; statistically true at $O(\log n)$ samples; false if any one of its three randomizations is dropped), and the authors explicitly invite cryptanalysis. [c/0105](/c/0105/) is the largest editorial step in the batch — the source names the direction and prints no statement, so the formalisation is the page's own and says so, along with the two choices it makes (the plain GV threshold rather than Gaborit–Zémor's logarithmic improvement, and the systematic form without which it is false). [c/0109](/c/0109/) turns a question into an existential proposition, flagged on the page, with the note that the negative answer settles it equally.

    **Four declines that were close and are worth not re-litigating.** 2021/1150's "does linear encoding complexity imply sub-linear minimum distance" is not stateable at the paper's strength — read literally over all binary linear codes it is already known false, so the intended class is one the paper never fixes. 2022/069's big-to-small Conjecture 14 is printed only informally and the paper says its own version may be "too strong/aggressive". 2023/1035's Conjecture 8 and 2024/252's multi-instance PPRF conjecture both fail the "settling it would change what is known" test: each buys a small constant in its own paper's parameters. 2020/814's non-composable incompressible encoding was the closest miss — a stated direction, but hedged to "at least under strong enough assumptions", and a 2020 paper in a live area needs a forward check this run did not carry to a conclusion.
  ...
  - **What's still waiting**, recounted 28 August 2026 after the batch above: **687** PDFs across 24 non-empty author subfolders, and seventeen folders that are now empty — Akshima, Alon Rosen, Andrej Bogdanov, Benny Applebaum, Boaz Barak, Daniel Wichs, Dominique Unruh, Geoffroy Couteau, Itai Dinur, Joel Alwen, Rafael Pass, Ron Rothblum, Siyao Guo, Vinod Vaikuntanathan, Yevgeniy Dodis, Yuval Ishai, Zvika Brakerski — because the 148 papers this batch read had been lifted out of them to the top level first. The remainder: Giulio Malavolta 82, Takashi Yamakawa 56, Aggelos Kiayias 44, Stefano Tessaro 43, Prabhanjan Ananth 39, Amit Sahai 38, Ilan Komargodski 37, Dakshita Khurana 35, Hoeteck Wee 33, Huijia Lin 32, Krzysztof Pietrzak 30, Aayush Jain 29, Tomoyuki Morimae 26, Qipeng Liu 23, Nir Bitansky 21, Alex Lombardi 20, Daniele Micciancio 16, Yanyi Liu 16, Alexander Russell 14, Divesh Aggarwal 13, Luowen Qian 12, Jens Groth 11, Jeremiah Blocki 11, Noah Stephens-Davidowitz 6. The top level of `latex/harvest/` is empty again, `processed/` holds 434 against 434 ledger documents, and `pruned/` holds 54. `scripts/draft_status.py` reports 110 drafts in `latex/conjectures/` against 106 pages in `c/` once this branch is merged onto a `main` that has since withdrawn c/0005 and c/0009, and **three** unpromoted drafts rather than the usual two: the pre-existing `polynomial-compatibility` and `simon-oracle-simultaneous-inversion`, plus `groth16`, which lost its page when c/0009 was folded into c/0008 and is now byte-identical to `split-nilp`. That third one is a consequence of the withdrawal, not of this batch, and wants a decision: repoint it or retire it. `prune-papers` has still not been run.
