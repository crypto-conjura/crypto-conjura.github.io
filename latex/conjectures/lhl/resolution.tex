% =====================================================================
%  SOLUTION DOCUMENT.  Resolves the public-seed variant of the LHL
%  conjecture stated in statement.tex in this folder (that document's
%  Remark 1; published, proved, on the live site as
%  https://crypto-conjura.github.io/c/0004/).  Along the way it
%  refutes the naive verbatim transposition of the secret-seed
%  conjecture to the public-seed game (Proposition~\ref{prop:false}).
%
%  Restyled onto the shared conjura-solution.cls house style -- see
%  latex/conjectures/_template/ -- from a plain-article draft, to
%  match the sibling k-cr and groth16 statements.  Only the preamble,
%  front matter, and bibliography changed; the mathematical content
%  (every definition, fact, lemma, theorem, proposition, corollary,
%  remark, and proof) is untouched.
% =====================================================================
...
% two theorem-like environments the shared class does not already provide;
% theorem/proposition/conjecture/corollary/lemma/definition/remark all come from the class
\theoremstyle{cjplain}
\newtheorem{fact}{Fact}
...
\runninghead{LHL RESOLUTION, PUBLIC SEED}

\begin{document}

\cjkicker{CONJURA \textperiodcentered{} SOLUTION}
\cjtitle{LHL for Random Functions}
\cjresolves{The public-seed extraction bound of \emph{Remark~1} in \texttt{statement.tex} in
  this folder, published as \href{https://crypto-conjura.github.io/c/0004/}{c/0004}. Along the
  way, refutes the naive verbatim transposition of the secret-seed conjecture to the
  public-seed game (Proposition~\ref{prop:false}).}
...
Figure~\ref{fig:games} and Definition~\ref{def:games}. The conjecture below is the statement
note's secret-seed conjecture, stated there for $\Adv^{\ext}$, transposed verbatim to
$\Adv^{\extp}$. The statement note also poses a separate, weaker conjecture for the public-seed
game; that one is treated in Corollary~\ref{cor:pub} and is not refuted.

\begin{conjecture}[companion note, transposed to the public-seed game]\label{conj:main}
...
alone, and $c = 2$ still suffices. Third, the statement note's own public-seed conjecture, which
already attaches the $1/K$ to the selection term, follows from Theorem~\ref{thm:main} with
$c = 2$ (Corollary~\ref{cor:pub}); so what
...
The statement note poses two conjectures, one for each game, and only the first is refuted here.
Its public-seed conjecture is not Conjecture~\ref{conj:main} but the weaker
...
\begin{conjurabibliography}{McD89}

\bibitem[McD89]{McD89}
Colin McDiarmid.
\emph{On the method of bounded differences}.
In Surveys in Combinatorics 1989, London Mathematical Society Lecture Note Series 141,
pages 148--188. Cambridge University Press, 1989.
Fact~\ref{fact:mcd} is the one-sided form of the \emph{independent bounded differences
inequality}, Lemma~(1.2) there, which is stated two-sidedly with a factor $2$; the one-sided
bound with the same constant is Theorem~(6.7), of which Lemma~(1.2) is the
independent-coordinate specialisation.
