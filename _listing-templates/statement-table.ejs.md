...
<td class="cj-tags">
<% for (const tp of (item.topics || [])) { %><a class="cj-tag cj-tag-topic" href="/problems/by-topic/<%= tp %>/"><%= tp.split('-').map(function(w){ return w.charAt(0).toUpperCase() + w.slice(1); }).join(' ') %></a><% } %><% if (item.model !== "standard") { %><span class="cj-tag"><%= item.model %></span><% } %><span class="cj-tag"><%= item.form %></span><% if (item.category !== "research-open") { %><span class="cj-tag listing-category"><%= item.category %></span><% } %><% if (item.difficulty && !(item.difficulty === "new-idea" && item.difficulty_by === "ai")) { %><span class="cj-tag cj-tag-difficulty" title="<%= item.difficulty_note %>"><%= item.difficulty %><% if (item.difficulty_by === "ai") { %> (ai)<% } %></span><% } %><% for (const tg of (item.tags || [])) { %><a class="cj-tag cj-tag-org" href="/problems/tags/<%= tg %>/" title="Provenance tag: an author of a cited source is affiliated with this organisation"><%= tg.toUpperCase() %></a><% } %>
</td>
</tr>
<% } %>
</tbody>
