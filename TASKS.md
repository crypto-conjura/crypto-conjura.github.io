...
- [ ] **Make the site deploy on Cloudflare (~1.5h — manual: the Cloudflare dashboard build settings are the user's to set, and third-party build-image troubleshooting is iterative)**

  Requested 17 August 2026. A Cloudflare deploy failed with `✘ [ERROR] Could not detect a directory containing static files (e.g. html, css and js) for the project`. The log shows the whole story: clone → `pip install -r requirements.txt` (auto-detected, installs PyYAML only) → `npx wrangler deploy`. **No build step ran at all.**

  Two independent causes, and fixing either alone still fails:

  - **Nothing renders the site.** This is a Quarto project (`_quarto.yml`, `output-dir: _site`) and `_site/` is gitignored (`.gitignore:1`), so the clone contains `.qmd` sources and zero HTML. Quarto is *not* in Cloudflare's build image — `publish.yml:60` gets it from `quarto-dev/quarto-actions/setup@v2` pinned to `1.10.18`, which has no equivalent there. The build command has to fetch the `linux-amd64` tarball from the `quarto-cli` GitHub releases and put it on `PATH` itself (verify the exact asset name against the release page rather than assuming it).
  - **Wrangler has no assets config.** There is no `wrangler.toml`/`wrangler.jsonc`/`package.json` anywhere in the repo (`git ls-files` confirms), so `wrangler deploy` sniffed for a static directory and found none. Either add a config with `name`, `compatibility_date` and `assets: { directory: "./_site" }`, or switch the deploy command to target the directory explicitly.

  **Reproduce the full GitHub Actions build, not just `quarto render`.** `publish.yml:33-58` runs six Python gates *before* rendering, and two of them generate files the render then consumes — `build_index.py` writes `conjura.json` and `_generated/`, both gitignored (`.gitignore:16-17`) and both listed as Quarto `resources:`. Skipping it doesn't error; it silently ships a site with empty facet listings. The correct build command mirrors `publish.yml` step for step: `status_badge.py --check c papers`, `build_index.py`, `check_relations.py`, `gen_interface.py --check`, `artifact_manifest.py --check`, `uc_status.py --check`, then `quarto render`. Note `_quarto.yml`'s `post-render` also shells out to `python3 scripts/strip_polyfill.py`, so Python must still be on `PATH` during the render itself.

  **The trap that will ship a broken site even after the deploy goes green:** `_quarto.yml:37` hardcodes `site-url: "https://crypto-conjura.github.io"`. That value is what emits `sitemap.xml`, canonical `<link>` tags and the absolute URLs in the Open Graph and Twitter card metadata — so a Cloudflare-hosted copy would advertise the GitHub Pages origin as canonical, which is worse than no canonical tag at all. Decide *before* deploying which origin is authoritative, and if it becomes Cloudflare, this needs to move to an env-var override rather than being flipped in place, or GitHub Pages builds start lying in the other direction.

  **Also decide what happens to GitHub Pages.** `publish.yml` still fires on every push to `main` and deploys to Pages. Two live deploy targets for the same content is a decision, not an accident — either retire the workflow, or keep both and make the canonical-URL call above explicit.

- [ ] **Watch `latex/conjectures/` for drafts with no `c/` counterpart, and scaffold the counterpart (~2.5h — manual: the editorial frontmatter and citation verification on each promotion)**

  Requested 17 August 2026. Nothing currently looks at `latex/conjectures/`. Every CI gate discovers content by globbing `c/` and `papers/` — `build_index.py:302`, `check_relations.py:86`, `status_badge.py --check c papers`, `artifact_manifest.py:62` (`c/0*`) — and `artifact_manifest.py:57` excludes the draft staging area on purpose: "Promoting one is a manual editorial act, not a build, so there is no output to be stale against." That reasoning holds for *staleness*; it does not cover *existence*, which is what goes unnoticed today. A harvested draft can sit in a tracked folder indefinitely with nothing reporting that it never became a page.

  One draft is in exactly that state right now, and it is the fixture to build against: `latex/conjectures/log-independence-prp/`, the harvester's first output, landed with the harvester itself in `14a7c91` (PR #50) and never promoted. Its `\cjtitle` is "Logarithmic Independence Implies Pseudorandomness for Local…". Ten drafts, nine pages.

  **The blocker to solve before writing any matching logic: there is no machine link between a draft and its page, and every candidate key fails on real data.** Checked against `main` at `14a7c91`:

  - **Folder name is not the `problem:` slug.** Every pair differs — `feistel-6round` → `feistel-6-round-indifferentiability` (`c/0001`), `mirror-theory` → `generalized-mirror-theory` (`0002`), `k-collisions-aux` → `k-collisions-auxiliary-input` (`0003`), `three-way-collision` → `three-way-collision-bounded-memory` (`0007`), `split-decomp` → `split-source-decomposition` (`0010`).
  - **`problem:` is not unique.** `0004`/`0005` share `leftover-hash-lemma-extraction`; `0008`/`0009` share `groth16-proof-size-optimality`. It names the *problem* a statement belongs to, not the statement.
  - **Titles are not unique either, and are actively misleading.** `groth16/statement.tex` and `split-nilp/statement.tex` carry byte-identical `\cjtitle{Groth16 Is the Limit in the Pure Generic Group Model}` *and* `\cjsubtitle{Two lower-bound conjectures on pairing-based SNARGs}` — yet they are two genuine, distinct statements published separately: `c/0008` is "No Two-Element **Split Non-Interactive Linear Proofs** for Hard Relations" and `c/0009` is "Groth16 Proof-Size Optimality in the Pure Generic Group Model". The pair was drafted as one document and split into two pages, so the shared title is the document's, not the statement's. Fuzzy title matching would confidently pair these backwards.
  - **The relation is not even 1:1, and one arm is unresolved.** `c/0004` is "…Public Seed" and `c/0005` is "…Secret Seed", but the drafts are `lhl-public-seed` and `lhl-k-source` ("…Public Seed, $k$-Source Generalization") — *no* draft corresponds to the secret-seed page. Whoever does this has to read the four documents and decide; it is not recoverable from metadata.

  So the task is to **add an explicit link and backfill it by hand** — and note the direction the evidence forces: since one draft can yield several pages, the field belongs on the page (`draft: <slug>` in `c/<id>/index.qmd`), not on the draft. Backfilling the nine existing pages is editorial work, not a scripted migration; the `lhl` arm above is the one that needs a judgment call before the check can mean anything.

  **The trap, and the main design decision.** Both workflows run every gate *before* `quarto render`, and a non-zero exit fails the whole job, so a hard-failing watcher would turn the site deploy red on every push by anyone until someone promoted `log-independence-prp` — the exact failure mode already recorded against `status_badge.py`. Make the existence check **report-only**, and reserve exit 1 for a link that is declared and broken (a `draft:` naming a folder that isn't there, or two pages claiming the same draft). Follow `scripts/uc_status.py` (139 lines): it is the closest precedent — derive what is derivable, let a human write the one judgment a script can't make, `--check` reports drift and writes nothing. Wire it into both `.github/workflows/checks.yml` and `publish.yml`.

  **The scaffolding half.** A draft is `statement.tex` + `conjura-conjecture.cls` in one flat folder (harvested ones also carry `.chktexrc`, `harvest.json` and `SOURCE.md`); a page is the shape of `c/0010/` — `index.qmd`, `latex/{main.tex,conjura-conjecture.cls,.chktexrc}`, `pdf/main.pdf`, and `lean/.gitkeep`, `sessions/.gitkeep`. Note `statement.tex` is renamed to `latex/main.tex` on the way in. For a harvested draft, `SOURCE.md` already carries the source paper, its sha256, every quote with its page, and the citations the adversarial pass could not verify — read it before writing `sources:`, since it is the evidence the frontmatter is supposed to summarize. Roughly half of `index.qmd`'s ~20 frontmatter fields are mechanical (`id`, `title`/`short_title` from `\cjtitle`, the folder skeleton, and `revision`/`statement_sha`/`status_badge`, which `status_badge.py` generates and must not be hand-written); the rest — `areas`, `model`, `form`, `assumption_class`, `category`, the six `status` flags, `difficulty.reach`/`note`, `status_summary`, `relations`, `sources` — are editorial and are where the estimate goes. Verify every citation is a real paper at a real venue in a real year rather than copying the draft's bibliography forward on trust.

  **Next id is `0011`.** `0006` is vacant and stays vacant: ids are allocate-once and never recycled (`CONTRIBUTING.md:93` records the deletion that freed it and that it was the last one permitted).
