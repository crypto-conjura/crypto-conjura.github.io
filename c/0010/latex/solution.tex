...
\item[(D)] $\kappa(q)\le5\sqrt{\sigma'\delta}+2\delta\sqrt M$ for every $q$ and
  ...
  Unconditional (Theorem~\ref{thm:D}). This bound mentions neither $q$ nor
  $\sigma$, and for every $q\ge1$ and $M\ge4$ it is the smaller of (B) and (D)
...
With
$\Lambda:=\min\bigl\{5\sqrt{\sigma'\delta},\ \sqrt{2\delta\ln(eN)}+4\delta\sqrt{\sigma'}\bigr\}$,
$$\kna(q)\ \le\ \Lambda+\mu(q)\quad\text{for every }q,\qquad
\kappa(0)\ \le\ \Lambda\ \text{ with no restriction on the observer.}$$
The second arm of $\Lambda$ is the smaller once $\delta\le0.85$, and is smaller by
...
\[
  2C_1=2M\ln2+2\ln4+4\ln N+2\ln\delta^{-1}\le1.3863M+2.7726+6\ln N
  \le1.3863M+3.4657\,\sigma',
\]
...
review at all. Its load-bearing new step is a single application of
