% =====================================================================
%  AICR - SELF-CONTAINED NOTE
%  The capping construction for c/0010, with the whole chain it rests on.
%  Requires aicr-solution.cls and aicr-conjecture.cls here.
% =====================================================================
...
\cjkicker{AICR \textperiodcentered{} SELF-CONTAINED NOTE}
...
The first is \emph{item 3} of a three-item lemma titled ``Numeric inequalities'', on p.~9 of the
ePrint. It is \textbf{not} numbered ``Lemma 4.3'', which exists in neither the ePrint nor the
published version; a citation in that form is unresolvable. Corrected after checking the card
against the paper.
\end{assumption}
...
\begin{remark}[the technique is classical; the two-source instance is not]\label{rem:vadhan}
Lemma~\ref{lem:B} is the two-source analogue of a textbook argument. Vadhan,
\emph{Pseudorandomness} (Foundations and Trends in TCS 7, 2012), Proposition~6.12, proves that a
random function extracts from a fixed flat source by exactly this route: a union bound over
\emph{all} $2^{M}$ subsets $T \subseteq [M]$, a Chernoff bound for each fixed $T$, and the
observation that the good event is a property of the function alone. Its Theorem~6.14 supplies the
$\binom{N}{K} \le (Ne/K)^{K}$ counting that produces $t_c$; and the remark following
Proposition~6.12 --- that the number of flat $k$-sources is $\binom{N}{K} \approx N^{K}$, a larger
double exponential --- is the same one-source-fails observation as
Remark~\ref{rem:onesource} below. (Both verified against the monograph.)

What is \emph{not} classical, and is where the novelty of this section sits, is
Lemma~\ref{lem:1} --- that the posterior of $\vx$ factorises because the sources are split, which
is what makes a rectangle available at all --- together with Lemma~\ref{lem:A}(a), whose
Cauchy--Schwarz across the two coordinates yields $\delta\sqrt{M}$ rather than $\sqrt{M\delta}$ and
has no one-source counterpart. Neither is available with a single source.
\end{remark}
