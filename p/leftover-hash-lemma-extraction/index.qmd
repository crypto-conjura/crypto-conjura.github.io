...
**The rate in $K$ is the same square root the conjecture asks for.** What separates them is what sits under it: the lemma pays $\sqrt{W}$, a *set size*, where the conjecture pays $\sqrt{\log_2 D}$, a *number of bits*. In seed length, $\log_2 W$ against $\log_2\log_2 D$:

| $D$ | $R$ | min-entropy | $\delta$ | $W$ | $\log_2 K$ conjectured | $\log_2 K$ by the lemma |
|---|---|---|---|---|---|---|
| $2^{256}$ | $2^{64}$ | 128 | $2^{-32}$ | $2^{64}$ | 72 | 126 |
| $2^{256}$ | $2^{128}$ | 128 | $2^{-64}$ | $2^{128}$ | 136 | 254 |
| $2^{256}$ | $2^{64}$ | 254 | $2^{-32}$ | $2^{2}$ | 72 | 63.6 |

The third row is the honest boundary: the loss appears only when $W$ is large, that is when the source has slack *and* the output is long. At $\epsilon = 1/D$ one has $W = 1$ and the bound is $0$ -- correctly, since a source of maximal entropy is uniform on all of $\calD$ and has nothing to select, which is the same phenomenon the [c/0004](/c/0004/) proof records when its own selection term vanishes at $t = D$.

The cause is the Cauchy--Schwarz step $\|v\|_1 \le \sqrt{KR}\,\|v\|_2$ inside the lemma. Support selection concentrates the deviation $v$ on one row out of $K$ instead of spreading it over all $KR$ cells, and there that step is off by about $\sqrt{RK}$: the true advantage for the source above is $O(1/K)$, while the lemma returns $\tfrac12\sqrt{(W-1)/K}$. That is why the optimal-extractor proofs work in $L_1$ instead, by concentration plus a union bound over flat sources; the [c/0004](/c/0004/) proof takes that route, arriving at it independently. (What is stated here is a limitation: the lemma cannot do better than this. Whether it does this well is not claimed.)
