...
| | secret | min-entropy | key | $\delta$ | salt, conjectured | salt, route | salt, universal |
|---|---|---|---|---|---|---|---|
| DH element in a 2048-bit group | $2^{2048}$ | 256 | 128 | $2^{-32}$ | **77** | 190 | 1920 |
...
The loss is the Cauchy--Schwarz step $\|v\|_1 \le \sqrt{KR}\,\|v\|_2$ inside the lemma, which is tight only when $v$ is spread evenly over all $KR$ cells; support selection puts a concentrated lump on one row instead. It is worth being careful about what that does *not* say: the true advantage against the constructed source is **not** $O(1/K)$, because the $K-1$ unpinned rows are not uniform either and contribute $\Theta(\sqrt{R/t})$, which dominates. The floor is a statement about what the technique can certify, not about how weak the source is. That is why the optimal-extractor proofs work in $L_1$ instead, by concentration plus a union bound over flat sources; the [c/0004](/c/0004/) proof takes that route, arriving at it independently. **This seed-length bound is not new.** It is the leftover hash lemma's second known limitation: Barak, Dodis, Krawczyk, Pereira, Pietrzak, Standaert and Yu, [*Leftover Hash Lemma, Revisited*](https://eprint.iacr.org/2011/088) (CRYPTO 2011), open by stating that an almost-universal family good enough for the LHL needs seed length at least $\min(u-v,\ v+2\log(1/\varepsilon)) - O(1)$ — in the notation here, $\min(\log_2 D - m,\ m + 2\log_2(1/\delta))$, which is both salt columns above as one bound — and attribute it to Stinson (1994). What that literature does *not* cover is this setting: it restricts to attackers "sufficiently limited as to not make the source $X$ dependent on the seed", which is exactly the hypothesis dropped here, and its random-oracle comparison is query-bounded. (One-sided throughout: the lemma cannot do better than this. Whether it does this well is not claimed — the certified ceiling is some $2^{21}$ above the floor.)
...
- [Barak, Dodis, Krawczyk, Pereira, Pietrzak, Standaert and Yu, Leftover Hash Lemma, Revisited, CRYPTO 2011 (ePrint 2011/088) -- states the LHL's seed-length limitation, attributed to Stinson (1994)](https://eprint.iacr.org/2011/088)
