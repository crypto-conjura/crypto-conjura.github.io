...
Ordered by difficulty: web-related tweaks, new web content, new conjectures, new papers, new books, resolutions. Time estimates assume Claude Opus 5 (Max reasoning effort) carries out ~99% of each task autonomously, with the user only prompting/directing rather than hand-implementing — keep new estimates calibrated the same way, and note some tasks have a real-world floor this doesn't shrink (audio/podcast production, LaTeX-toolchain trial runs) even under full automation. Total estimated time: **~38h** (rough; the UC Encyclopedia content item is especially uncertain and could run well over its estimate).

- [ ] **Usability on iPhone, Android, and other handheld devices (~1.5h)**
...
- [ ] **Site-wide uniformity audit: how a page relates to its position in the site (~3h)**
...
- [ ] **Document the pre-human review pipeline on the Programme Committee page (~0.5h)**
...
- [ ] **A Philosophy page, with an audio read-out and a podcast (~3h)**
...
- [ ] **Convert the UC-for-gamers paper to HTML (~3h)**
...
- [ ] **Interactive UC tutorial for the website (~10h)**
