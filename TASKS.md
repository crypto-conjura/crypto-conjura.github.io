...
- [ ] **Build a StackExchange-style topic tag system: standardized, clickable, per-tag listing pages (~7h — the taxonomy design and retagging 48 statements by hand are real editorial work, not mechanical; the rest is small)**

  Requested 19 August 2026, with an explicit instruction to be pedantic and to verify the result in a real browser before calling it done — do both when this is picked up.

  **The decision to make first, because it changes the file list below.** Researched before logging: the site already has *two* pieces that look like what was asked for, and neither is it.

  - `areas:` (`scripts/build_index.py`'s `AREA_SLUGS`, 19 fixed values — foundations, lattices, quantum, zk, etc.) is a closed, schema-validated taxonomy with the full generator/listing-page machinery already built and working: one `_generated/areas/<value>.yml` per value, one `problems/by-area/areas/<value>/index.qmd` listing page per value (built 19 August 2026, this session). But it is coarse — 19 broad categories, at most 1–3 per statement — and, checked while scoping this, **`areas` is never actually rendered anywhere on the site**: not as a badge in the statement table (`_listing-templates/statement-table.ejs.md` renders `model`/`form`/`category`/`difficulty` as plain, non-clickable pills, lines ~24–46, but not `areas`), and not on an individual `c/*/index.qmd` page body either (checked `c/0001`, `c/0047`: no area badge anywhere).
  - `tags:` (`build_index.py`'s `TAG_SLUGS`, currently exactly one value, `"iog"`) is a *provenance* tag, not a topic tag, by explicit design — the code's own comment distinguishes "an area says what a statement is about; a tag says where it came from." It is used by exactly 3 of 48 statements (`c/0039`, `c/0040`, `c/0041`) and *is* already rendered as a clickable pill (`.cj-tag-org` in `theme-{light,dark}.scss`, linking to `/problems/tags/iog/`) — so the clickable-chip mechanism this request wants already exists and works, it's just wired to a one-value provenance concept rather than a topic taxonomy.

  What StackExchange tags actually are — several per question, drawn from a large, curated, growable vocabulary, each often with its own short description — is closer to neither of the above: coarser than a real tag vocabulary if reusing `areas`, and semantically the wrong concept if reusing `tags:`. **Recommendation: build a third, new field** (e.g. `topics:`, a list) with its own closed-but-growable enum (same discipline as `AREA_SLUGS`/`TAG_SLUGS` — a free-text tag field "grows typos and near-synonyms," per the existing code comment, and nothing would catch it), reusing the existing `.cj-tag`/`.cj-tag-org` chip CSS rather than inventing new styling. Decide this explicitly before starting; the alternative (retrofitting `areas` with badges and calling it done) is far cheaper (~1h, mechanical) but is not what was asked for and should not be substituted silently.

  **What full implementation touches**, if the recommendation above is taken:

  - `scripts/build_index.py`: a `TOPICS` enum, frontmatter validation, and a `_generated/topics/<value>.yml` writer (copy the existing `areas`/`model`/`form` pattern at lines ~276–296).
  - One `problems/by-topic/<value>/index.qmd` per topic value (copy the `problems/by-area/areas/<value>/index.qmd` pattern from 19 August), plus **one browse-all-tags overview page** with a tag cloud and per-tag statement counts — the StackExchange `/tags` page is the model to match, and this site currently has no equivalent for *any* facet, areas included.
  - `_listing-templates/statement-table.ejs.md`: render `topics` as clickable chips in the listing table (currently no facet is clickable there at all).
  - Every `c/*/index.qmd` page body: render its own `topics` as clickable chips — currently nothing renders on an individual statement page besides the status badge.
  - **All 48 `c/*/index.qmd` files need real `topics:` values.** This is the part that isn't mechanical: it means reading each statement and assigning the topics it actually belongs to (e.g. `log-rank-conjecture`, `sum-of-squares`, `generic-group-model`, `feistel-networks`, `black-box-separations`, `signature-schemes`, `quantum-information` — deciding the initial vocabulary is itself part of the work, and it will and should overlap with `areas`/`model` in places without duplicating them).
  - Optional but genuinely StackExchange-like, and worth doing given "be pedantic": a one-paragraph description on each tag's listing page, the way SE tags carry a tag wiki excerpt.

  **Verification, per the explicit instruction, is not optional and is not `curl`.** Run a full `quarto render` (never `quarto render <single-file>.qmd` — see the standing gotcha about it scattering build clutter across the whole tree), then drive an actual browser against the rendered output or `quarto preview` — Playwright with a real Chromium build (already used successfully for mobile-device checks on this site) is the established way to do this here. Confirm concretely: a tag chip is a real link, not styled text; clicking it lands on a listing page that contains only statements carrying that tag; the listing page's count matches the number of `c/*/index.qmd` files with that value; and both light and dark themes render the chips correctly.
